...
import ext.mods.gameserver.model.actor.player.EmailState;
import ext.mods.gameserver.model.actor.player.UiTempState;
		...
		if (command.startsWith("email"))
		{
			...
			switch (param.toLowerCase())
			{
				...
				case "target":
					...
					EmailState.setTarget(player, targetName);
					PlayerEmailManager.getInstance().navi(player, "email_main.htm");
					break;
				
				case "set_duration":
					String duration = tokenizer.getToken(2);
					EmailState.setDuration(player, duration);
					player.sendMessage("Tempo de expiração definido para: " + duration);
					PlayerEmailManager.getInstance().navi(player, "email_main.htm");
					break;
				
				case "clean":
					EmailState.setTarget(player, null);
					PlayerEmailManager.getInstance().navi(player, "email_main.htm");
					break;
				...
				case "finalize_item":
					...
					if (finalItem == null || qtd < 1 || qtd > finalItem.getCount())
					{
						...
						useVoicedCommand("email confirm_select_item " + objectIdFinal, player, target);
						return true;
					}
					
					player.sendMessage("Selecionado " + qtd + "x " + finalItem.getName());
					UiTempState.addSelectedItem(player, objectIdFinal, qtd);
				...
				case "confirm_select_item":
					...
					Map<Integer, Long> selectedIds = UiTempState.selectedItems(player);
				...
				case "clear_all_items":
					UiTempState.clearSelectedItems(player);
				...
				case "remove_selected_item":
					if (tokenizer.countTokens() >= 2)
					{
						...
						UiTempState.removeSelectedItem(player, objectId);
