...
public class BattleInstance implements ITournamentBattle
{
	...
	{
		TournamentHooks.get().savePlayerState(player);
		player.setInstanceMap(instance, true);
		TournamentState.setBattle(player, this);
		...
		TournamentState.setOpponents(player, new ArrayList<>(opponents));
	}
	...
	private void cleanUp()
	{
		...
		for (Player player : _players)
		{
			if (player != null)
			{
				TournamentHooks.get().restorePlayerState(player);
	...
	private void showOpponentStatus()
	{
		...
		for (Player player : _players)
		{
			...
			for (Player opponent : TournamentState.opponents(player))
			{
