...
public class VoicedTour implements IVoicedCommandHandler
{
	...
	public static void navi(Player player)
	{
		...
		content = content.replace("%registed%", TournamentState.isIn(player) ? "ON" : "OFF");
	...
	private void registerToTournament(Player player)
	{
		if (TournamentEvent.isRunningRegister())
		{
			...
			if (player.getParty() != null)
			{
				...
				{
					if (!TournamentState.isIn(member))
					{
						TournamentState.setIn(member, true);
			...
			else
			{
				...
				{
					TournamentState.setIn(player, true);
		...
		if (TournamentEvent.isRunningRegister())
		{
			if (player.getParty() != null)
			{
				...
				for (Player member : player.getParty().getMembers())
				{
					if (TournamentState.isIn(member))
					{
						TournamentState.setIn(member, false);
