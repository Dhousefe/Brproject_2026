...
public class GlobalDropManager
{
	...
	public void onKill(Player player, Attackable monster)
	{
		...
		for (DropHolder drop : drops)
		{
			...
			{
				if (ConfigServer.DEVELOPER) { LOGGER.info("[Debug GlobalDrop] SUCCESS! Dropping item " + drop.getItemId() + " (Chance: " + effectiveChance + ")"); }
				...
				if (player.isInParty())
				{
					...
					{
					    if (member != null && member.isOnline() && player.isIn3DRadius(member, ConfigPlayers.PARTY_RANGE))
					    {
			...
			else if (ConfigServer.DEVELOPER)
			{
