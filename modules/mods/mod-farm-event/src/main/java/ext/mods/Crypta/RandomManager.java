...
public class RandomManager
{
	...
	{
		if (ConfigServer.DEVELOPER) { LOGGER.info("[Debug Event] activateZonesAndSpawn: Activating zones..."); }
	...
	{
		if (ConfigServer.DEVELOPER) { LOGGER.info("[Debug Spawn] spawnEventMonsters: Starting..."); }

		final int pauseDelay = getEventPauseDelay();
		
		if (ConfigServer.DEVELOPER) { LOGGER.info("[Debug Spawn] Pausing " + oldMonsters.size() + " old monsters from active zones."); }
		...
		for (RandomZone zone : activeZones) {
			...
			if (ConfigServer.DEVELOPER) { LOGGER.info("[Debug Spawn] Spawning for Zone ID " + zone.getId() + " (useOriginals=" + zoneData.useOriginals() + ")"); }
				...
				for (Npc oldNpc : oldMonsters) {
					if (oldNpc != null && oldNpc.getSpawn() != null) {
						...
						if (zone.isInsideZone(loc.getX(), loc.getY(), loc.getZ())) {
							...
							if (ConfigServer.DEVELOPER) { LOGGER.info("[Debug Spawn] MODE 1/3: Re-spawning original NPC " + oldNpc.getNpcId() + " with default title: " + title + " and respawn: " + respawnDelay); }
							try {
			...
			for (RandomSpawnHolder holder : zoneData.getCustomSpawns()) {
				...
				if (ConfigServer.DEVELOPER) { LOGGER.info("[Debug Spawn] MODE 2/3: Spawning custom NPC " + holder.getNpcId() + " Count: " + holder.getCount()); }
	...
	private void cleanPreviousMonsters() {  
		...
		if (ConfigServer.DEVELOPER) { LOGGER.info("[Debug Event] cleanPreviousMonsters: Saved " + oldMonsters.size() + " old NPCs from active zones."); }
	}

	private void stopZones() {
		if (ConfigServer.DEVELOPER) { LOGGER.info("[Debug Event] Event STOPPING. Cleaning up zones."); }
		...
		if (ConfigServer.DEVELOPER) { LOGGER.info("[Debug Event] Deleting " + activeSpawns.size() + " event spawns."); }
		...
		activeSpawns.clear();
		if (ConfigServer.DEVELOPER) { LOGGER.info("[Debug Event] Re-spawning " + oldMonsters.size() + " old monsters."); }
	...
	public void onKill(Player player, Attackable monster) {  
		if (player == null || monster == null) return; if (ConfigServer.DEVELOPER) { LOGGER.info("[Debug Kill] " + player.getName() + " killed " + monster.getName()); }
		...
		if (isEventMonster && isRunning && timeRemaining > 0) {
			if (ConfigServer.DEVELOPER) { LOGGER.info("[Debug Kill] Monster " + monster.getNpcId() + " is an event monster. Checking drops..."); }
			...
			if (zoneData == null) { if (ConfigServer.DEVELOPER) { LOGGER.warn("[Debug Kill] No ZoneData found for the zone monster is in!"); } return; }
			...
			if (customSpawnData != null && !customSpawnData.getDrops().isEmpty()) { if (ConfigServer.DEVELOPER) { LOGGER.info("[Debug Kill] Monster is CUSTOM spawn. Using custom drops."); } dropsToUse = customSpawnData.getDrops(); }
			else if (zoneData.useOriginals() && zoneData.dropsOriginals()) { if (ConfigServer.DEVELOPER) { LOGGER.info("[Debug Kill] Monster is ORIGINAL spawn and DropsOriginals=true. Using default zone drops."); } dropsToUse = zoneData.getDefaultDrops(); }
			if (dropsToUse != null && !dropsToUse.isEmpty()) {
				for (DropHolder drop : dropsToUse) {
					if (Rnd.get(100) < drop.getChance()) {
						if (ConfigServer.DEVELOPER) { LOGGER.info("[Debug Kill] SUCCESS! Dropping item " + drop.getItemId() + " (Chance: " + drop.getChance() + ")"); }
						...
						if (player.isInParty()) {
							...
							for (Player member : members) { if (member != null && member.isOnline() && player.isIn3DRadius(member, ConfigPlayers.PARTY_RANGE)) { int amount = baseAmount; if (remainder > 0) { amount++; remainder--; } if (amount > 0) member.addItem(drop.getItemId(), amount, true); } else if (remainder > 0) { remainder--; } }
							if (remainder > 0) { player.addItem(drop.getItemId(), remainder, true); }
						} else { player.addItem(drop.getItemId(), totalAmount, true); }
					} else if (ConfigServer.DEVELOPER) { LOGGER.info("[Debug Kill] FAILED drop for item " + drop.getItemId() + " (Chance: " + drop.getChance() + ")"); }
				}
			} else if (customSpawnData == null && zoneData.useOriginals() && !zoneData.dropsOriginals()) { if (ConfigServer.DEVELOPER) { LOGGER.info("[Debug Kill] Monster is ORIGINAL spawn and DropsOriginals=false. No event drops added."); } }
			else if (ConfigServer.DEVELOPER){ LOGGER.warn("[Debug Kill] No drops configured or applicable for this monster!"); }
		}
	}
	...
	public double getActiveRateXp(Attackable monster) {  
		if (!isRunning || monster == null || config == null) return 1.0; boolean isEventMonster = activeSpawns.stream().anyMatch(spawn -> spawn != null && monster.getSpawn() == spawn); if (!isEventMonster) return 1.0; for (RandomZone zone : activeZones) { if (zone.isInsideZone(monster)) { RandomZoneData zoneData = RandomData.getInstance().getZoneData(config.getName(), zone.getId()); if (zoneData == null) return 1.0; RandomSpawnHolder customSpawnData = zoneData.getCustomSpawns().stream().filter(s -> s.getCount() > 0 && activeSpawns.stream().anyMatch(as -> as == monster.getSpawn() && as.getTemplate().getNpcId() == s.getNpcId())).findFirst().orElse(null); if (customSpawnData != null && customSpawnData.getRateXp() > -1.0) { if(ConfigServer.DEVELOPER) LOGGER.info("[Debug Rate] Usando rateXP específico do Spawn {} para NPC {}", customSpawnData.getRateXp(), monster.getNpcId()); return customSpawnData.getRateXp(); } if(ConfigServer.DEVELOPER) LOGGER.info("[Debug Rate] Usando rateXP padrão da Zona {} ({}) para NPC {}", zoneData.getRateXp(), zone.getName(), monster.getNpcId()); return zoneData.getRateXp(); } } return 1.0;
	}
	public double getActiveRateSp(Attackable monster) {  
		if (!isRunning || monster == null || config == null) return 1.0; boolean isEventMonster = activeSpawns.stream().anyMatch(spawn -> spawn != null && monster.getSpawn() == spawn); if (!isEventMonster) return 1.0; for (RandomZone zone : activeZones) { if (zone.isInsideZone(monster)) { RandomZoneData zoneData = RandomData.getInstance().getZoneData(config.getName(), zone.getId()); if (zoneData == null) return 1.0; RandomSpawnHolder customSpawnData = zoneData.getCustomSpawns().stream().filter(s -> s.getCount() > 0 && activeSpawns.stream().anyMatch(as -> as == monster.getSpawn() && as.getTemplate().getNpcId() == s.getNpcId())).findFirst().orElse(null); if (customSpawnData != null && customSpawnData.getRateSp() > -1.0) { if(ConfigServer.DEVELOPER) LOGGER.info("[Debug Rate] Usando rateSP específico do Spawn {} para NPC {}", customSpawnData.getRateSp(), monster.getNpcId()); return customSpawnData.getRateSp(); } if(ConfigServer.DEVELOPER) LOGGER.info("[Debug Rate] Usando rateSP padrão da Zona {} ({}) para NPC {}", zoneData.getRateSp(), zone.getName(), monster.getNpcId()); return zoneData.getRateSp(); } } return 1.0;
	}
	public double getActiveRateAdena(Attackable monster) {  
		if (!isRunning || monster == null || config == null) return 1.0; boolean isEventMonster = activeSpawns.stream().anyMatch(spawn -> spawn != null && monster.getSpawn() == spawn); if (!isEventMonster) return 1.0; for (RandomZone zone : activeZones) { if (zone.isInsideZone(monster)) { RandomZoneData zoneData = RandomData.getInstance().getZoneData(config.getName(), zone.getId()); if (zoneData == null) return 1.0; RandomSpawnHolder customSpawnData = zoneData.getCustomSpawns().stream().filter(s -> s.getCount() > 0 && activeSpawns.stream().anyMatch(as -> as == monster.getSpawn() && as.getTemplate().getNpcId() == s.getNpcId())).findFirst().orElse(null); if (customSpawnData != null && customSpawnData.getRateAdena() > -1.0) { if(ConfigServer.DEVELOPER) LOGGER.info("[Debug Rate] Usando rateAdena específico do Spawn {} para NPC {}", customSpawnData.getRateAdena(), monster.getNpcId()); return customSpawnData.getRateAdena(); } if(ConfigServer.DEVELOPER) LOGGER.info("[Debug Rate] Usando rateAdena padrão da Zona {} ({}) para NPC {}", zoneData.getRateAdena(), zone.getName(), monster.getNpcId()); return zoneData.getRateAdena(); } } return 1.0;
	}
	...
	private void startPartyCheckTask() {
		...
		if (ConfigServer.DEVELOPER) { LOGGER.info("[Debug Party Check] Tarefa iniciada."); }
	}
	...
	private void checkPartyIntegrity() {
		if (!isRunning || activeZones.isEmpty()) {
			if (_partyCheckTask != null && !_partyCheckTask.isDone()) {
				_partyCheckTask.cancel(true);
				if (ConfigServer.DEVELOPER) { LOGGER.info("[Debug Party Check] Tarefa cancelada (evento inativo)."); }
			}
			return;
		}

		if (ConfigServer.DEVELOPER) { LOGGER.info("[Debug Party Check] Verificando integridade das parties..."); }
		...
		for (RandomZone zone : activeZones) {
			...
			if (ConfigServer.DEVELOPER) { LOGGER.info("[Debug Party Check] Verificando Zona: " + zone.getName() + " (ID: " + zone.getId() + ")"); }
			...
			for (Player player : playersInside) {
				...
				if (party == null) {
					...
					if (ConfigServer.DEVELOPER) { LOGGER.info("[Debug Party Check] Player " + player.getName() + " não está mais em party."); }
				} 
					...
					if (ConfigServer.DEVELOPER) { LOGGER.info("[Debug Party Check] Party de " + player.getName() + " tem menos que " + zoneData.getMinPartySize() + " membros."); }
				} 
					...
					for (Player member : party.getMembers()) {
						...
						if (member == null || !zone.isInsideZone(member)) {
							...
							if (ConfigServer.DEVELOPER) { LOGGER.info("[Debug Party Check] Membro " + (member != null ? member.getName() : "NULO") + " da party de " + player.getName() + " não está na zona."); }
							break; 
						}
					}
				}
				...
				if (expelParty) {
					if (party != null) {
						...
						if (ConfigServer.DEVELOPER) { LOGGER.info("[Debug Party Check] Expulsando party do líder " + party.getLeader().getName() + " da zona " + zone.getName() + ". Razão: " + reason); }
						...
						if (ConfigServer.DEVELOPER) { LOGGER.info("[Debug Party Check] Expulsando jogador " + player.getName() + " (sem party) da zona " + zone.getName() + ". Razão: " + reason); }
				    ...
				    processedPartyLeaders.add(party.getLeaderObjectId());
				    if (ConfigServer.DEVELOPER) { LOGGER.info("[Debug Party Check] Party de " + player.getName() + " OK na zona " + zone.getName()); }
				}
			}
		}
	}
	...
	private void sendTeleportConfirmationToAllPlayers()
	{
		...
		for (Player player : World.getInstance().getPlayers())
		{
			...
			{
				if (player.isDead() || player.isInOlympiadMode() || DungeonState.get(player) != null)
					continue;
