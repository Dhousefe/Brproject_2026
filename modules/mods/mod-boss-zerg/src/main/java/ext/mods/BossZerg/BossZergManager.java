...
public final class BossZergManager
{
	...
	private void start()
	{
		if (!ConfigBossZerg.BOSS_ZERG_ENABLED)
			return;
		
		final long interval = Math.max(1000L, ConfigBossZerg.BOSS_ZERG_CHECK_INTERVAL_MS);
	...
	private void processBoss(Npc boss, BossZergState state)
	{
		...
		final Map<Integer, PartyPresence> parties = collectPartiesNearBoss(boss, ConfigBossZerg.BOSS_ZERG_RANGE);
		...
		{
			if (presence.players.size() < ConfigBossZerg.BOSS_ZERG_MIN_PARTY_SIZE)
				continue;
		...
		final List<Player> playersInRange = boss.getKnownTypeInRadius(Player.class, ConfigBossZerg.BOSS_ZERG_RANGE);
		...
		for (Map.Entry<Integer, List<Player>> entry : groupMap.entrySet())
		{
			...
			if (members.size() > ConfigBossZerg.BOSS_ZERG_MAX_ALLY_MEMBERS)
			{
				members.sort(Comparator.comparingLong(player -> state.playerEntryTimes.getOrDefault(player.getObjectId(), 0L)));

				for (int i = ConfigBossZerg.BOSS_ZERG_MAX_ALLY_MEMBERS; i < members.size(); i++)
				{
	...
	private void teleportExcessPlayer(Player player, String bossName) {
		...
		player.sendMessage("Sua Aliança/Clã excedeu o limite de " + ConfigBossZerg.BOSS_ZERG_MAX_ALLY_MEMBERS + " membros para o " + bossName + ".");
	...
	{
		if (!ConfigBossZerg.BOSS_ZERG_FLAG_ENABLED)
		{
			clearFlagPreview(state);
			return;
		}
		
		final int range = Math.max(0, ConfigBossZerg.BOSS_ZERG_FLAG_RANGE);
		if (range == 0)
		{
			clearFlagPreview(state);
			return;
		...
		final long flagTime = Math.max(1000L, ConfigBossZerg.BOSS_ZERG_FLAG_RENEW_MS);
		final List<Player> nearbyPlayers = boss.getKnownTypeInRadius(Player.class, range);
		final int minPlayers = Math.max(1, ConfigBossZerg.BOSS_ZERG_FLAG_MIN_PLAYERS);
		
		if (ConfigBossZerg.BOSS_ZERG_FLAG_AREA_ENABLED)
		{
			final int warningRange = Math.max(0, ConfigBossZerg.BOSS_ZERG_FLAG_WARNING_RANGE);
			...
			{
				final Color previewColor = new Color(ConfigBossZerg.BOSS_ZERG_FLAG_AREA_COLOR);
				final int previewRadius = Math.max(0, ConfigBossZerg.BOSS_ZERG_FLAG_AREA_RADIUS);
	...
	{
		if (ignoreBoss || !ConfigBossZerg.BOSS_ZERG_SHOW_ALLY_COUNT || groupMap.isEmpty())
			return;
		
		for (Map.Entry<Integer, List<Player>> entry : groupMap.entrySet())
		{
			final int groupSize = entry.getValue().size();
			final String message = "Ally/Clan no Boss: " + groupSize + "/" + ConfigBossZerg.BOSS_ZERG_MAX_ALLY_MEMBERS;
	...
	{
		return ConfigBossZerg.BOSS_ZERG_IGNORED_BOSS_IDS.contains(boss.getNpcId());
	}
	...
	{
		if (!ConfigBossZerg.BOSS_ZERG_ANNOUNCE)
			return;
		...
		if (now - lastAnnounce < ConfigBossZerg.BOSS_ZERG_ANNOUNCE_COOLDOWN_MS)
			return;
	...
	{
		if (!ConfigBossZerg.BOSS_ZERG_SHOW_AREA)
		{
			clearPreview(state);
			return;
		}
		...
		final int radius = Math.max(0, ConfigBossZerg.BOSS_ZERG_AREA_RADIUS);
		if (radius == 0)
			return;
	...
	{
		if (!ConfigBossZerg.BOSS_ZERG_HEAL_PENALTY_ENABLED || amount <= 0)
			return amount;
		...
		double result = amount * ConfigBossZerg.BOSS_ZERG_HEAL_PENALTY_MULTIPLIER;
		
		if (hasPrayerEffect(targetPlayer))
			result *= ConfigBossZerg.BOSS_ZERG_PRAYER_REVERSE_MULTIPLIER;
	...
	{
		if (player == null || ConfigBossZerg.BOSS_ZERG_PRAYER_SKILL_IDS.length == 0)
			return false;
		
		for (int skillId : ConfigBossZerg.BOSS_ZERG_PRAYER_SKILL_IDS)
		{
