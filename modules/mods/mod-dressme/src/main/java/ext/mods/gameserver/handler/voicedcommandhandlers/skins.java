	...
	public boolean useVoicedCommand(String command, Player player, String target)
	{
		...
		while (st.hasMoreTokens())
		{
			...
			else if ("apply".equalsIgnoreCase(token) && st.hasMoreTokens())
			{
				...
				if (currentTime - DressMeState.lastSummonTime(player) < cooldown)
				{
				...
				if (skin != null)
				{
					...
					{
						DressMeHooks.get().apply(player, skin, true);
						player.sendMessage("Skin aplicada: " + skin.getName());
						DressMeState.setLastSummonTime(player, currentTime);
					}
				}
			}
			else if ("preview".equalsIgnoreCase(token) && st.hasMoreTokens())
			{
				...
				if (skin != null)
				{
					DressMeHooks.get().apply(player, skin, false);
					player.sendMessage("Visual temporário: " + skin.getName());
					ThreadPool.schedule(() ->
					{
						DressMeHooks.get().removeArmor(player);
						DressMeHooks.get().removeWeapon(player);
					}, 1L * 60L * 1000L);
					DressMeState.setLastSummonTime(player, currentTime);
				}
			}
