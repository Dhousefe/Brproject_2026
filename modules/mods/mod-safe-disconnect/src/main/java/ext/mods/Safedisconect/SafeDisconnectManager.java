...
public final class SafeDisconnectManager
{
	...
	{
		if (!ConfigSafeDisconnect.SAFEDISCONNECT_ENABLED || client == null)
			return false;
	...
	public void onEnterWorld(Player player)
	{
		...
		if (session != null)
		{
			...
			if (ConfigSafeDisconnect.SAFEDISCONNECT_INTEGRATION_DUNGEON && DungeonState.get(player) != null)
	...
	private void ensureInstanceConsistency(Player player)
	{
		...
		final boolean dungeonMissing = DungeonState.get(player) == null;
		final boolean battleMissing = TournamentState.battle(player) == null;
		
		if (registered == null || (dungeonMissing && battleMissing))
		{
			DungeonState.set(player, null);
	...
	private void pauseOlympiadIfNeeded(Player player)
	{
		if (player == null)
			return;
		if (!ConfigSafeDisconnect.SAFEDISCONNECT_INTEGRATION_OLYMPIAD)
			return;
	...
	private void applySafeState(Player player, SafeDisconnectSession session)
	{
		...
		if (ConfigSafeDisconnect.SAFEDISCONNECT_INTEGRATION_DUNGEON && DungeonState.get(player) != null)
			DungeonState.get(player).pauseForDisconnect(player);
		if (ConfigSafeDisconnect.SAFEDISCONNECT_INTEGRATION_TOURNAMENT && TournamentState.battle(player) != null)
			TournamentState.battle(player).pauseForDisconnect(player);
		...
		if (ConfigSafeDisconnect.SAFEDISCONNECT_IMMOBILIZE)
			player.setIsImmobilized(true);
		
		if (ConfigSafeDisconnect.SAFEDISCONNECT_INVULNERABLE)
			player.setInvul(true);
		
		if (ConfigSafeDisconnect.SAFEDISCONNECT_CHANGE_NAME_COLOR)
			player.getAppearance().setNameColor(ConfigSafeDisconnect.SAFEDISCONNECT_NAME_COLOR);
		
		player.setTitle(ConfigSafeDisconnect.SAFEDISCONNECT_TITLE);
		player.broadcastUserInfo();
	}
	...
	private void restoreOriginalAppearance(Player player, SafeDisconnectSession session)
	{
		...
		else if (ConfigSafeDisconnect.SAFEDISCONNECT_CHANGE_NAME_COLOR)
		{
	...
	private void restoreAutoFarmState(Player player)
	{
		...
		final AutoFarmManager autoFarmManager = AutoFarmManager.getInstance();
		final long delay = Math.max(0L, ConfigSafeDisconnect.SAFEDISCONNECT_AUTOFARM_RESTORE_DELAY_MS);
		if (delay == 0L)
		{
	...
	private boolean canApplySafeDisconnect(Player player)
	{
		...
		if (player.isInOlympiadMode() && !ConfigSafeDisconnect.SAFEDISCONNECT_ALLOW_OLYMPIAD)
			return false;
		
		if (player.isInsideZone(ZoneId.BOSS) && !ConfigSafeDisconnect.SAFEDISCONNECT_ALLOW_BOSS)
			return false;
		
		if (!ConfigSafeDisconnect.SAFEDISCONNECT_ALLOW_QUEST && !player.getQuestList().getAllQuests(false).isEmpty())
			return false;
		
		if (!ConfigSafeDisconnect.SAFEDISCONNECT_ALLOW_FARM && player.isInCombat())
			return false;
	...
	private class SafeDisconnectSession
	{
		...
		{
			if (ConfigSafeDisconnect.SAFEDISCONNECT_TIMEOUT_MS <= 0)
				return;
			
			timeoutTask = CoroutinePool.schedule(() -> CoroutinePool.executeParallel(() -> onTimeout(this)), ConfigSafeDisconnect.SAFEDISCONNECT_TIMEOUT_MS);
		}
