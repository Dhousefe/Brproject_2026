...
public class BattleBossRumble implements Runnable
{
	...
	private void startBattle()
	{
		...
		for (Player p : snapshotParticipants())
		{
			...
			if (p.isFakeDeath())
				p.stopFakeDeath(true);
			BattleBossState.setRumbleId(p, rumbleId);
			BattleBossState.setEventId(p, event.getId());
		}
	...
	private void teleportBack(List<Player> players)
	{
		...
		{
			setInvulParalyze(p, true);
			BattleBossState.setRumbleId(p, 0);
			BattleBossState.leave(p);
	...
	private void safeReturnAndDetach(Player member)
	{
		...
		try
		{
			...
			BattleBossState.setRumbleId(member, 0);
			BattleBossState.leave(member);
		}
		catch (Exception e)
		{
