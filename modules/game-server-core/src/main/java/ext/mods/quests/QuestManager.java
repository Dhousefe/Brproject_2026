...
public class QuestManager
{
	public void showMenuQuest(Player player, int page)
	{
		...
		for (int questId : QuestKillState.activeQuestIds(player))
		{
			...
			if (!quest.getObjectivesForClass(playerClassId).isEmpty())
			{
				...
				for (QuestObjective obj : quest.getObjectivesForClass(playerClassId))
				{
					...
					long currentCount = QuestKillState.getKillCount(player, questId, npcId);
					...
					if (QuestKillState.isCompleted(player, questId))
					{
			...
			sb.append("<button action=\"bypass -h questnotify html ").append(QuestKillState.isNotifyHtml(player) ? "off" : "on").append("\" ").append("width=16 height=16 back=\"").append(QuestKillState.isNotifyHtml(player) ? "L2UI.CheckBox_checked" : "L2UI.CheckBox").append("\" ").append("fore=\"").append(QuestKillState.isNotifyHtml(player) ? "L2UI.CheckBox_checked" : "L2UI.CheckBox").append("\">");
			sb.append("</td>");
			sb.append("</tr>");
			...
			sb.append("<td width=16 align=center>");
			sb.append("<button action=\"bypass -h questnotify chat ").append(QuestKillState.isNotifyChat(player) ? "off" : "on").append("\" ").append("width=16 height=16 back=\"").append(QuestKillState.isNotifyChat(player) ? "L2UI.CheckBox_checked" : "L2UI.CheckBox").append("\" ").append("fore=\"").append(QuestKillState.isNotifyChat(player) ? "L2UI.CheckBox_checked" : "L2UI.CheckBox").append("\">");
			sb.append("</td>");
			sb.append("</tr>");
	...
	public void showCompleteQuest(Player player, QuestHolder quest, int page)
	{
		...
		if (!quest.getObjectivesForClass(playerClassId).isEmpty())
		{
			...
			for (QuestObjective obj : quest.getObjectivesForClass(playerClassId))
			{
				...
				if (QuestKillState.isCompleted(player, quest.getId()))
				{
