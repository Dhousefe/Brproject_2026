	...
	public void run()
	{
		...
		while (true)
		{
			try
			{
				...
				if (ConfigLogin.FLOOD_PROTECTION)
				{
					...
					{
						fc.attempts += 1;
						if ((fc.attempts > ConfigLogin.FAST_CONNECTION_LIMIT && (currentTime - fc.lastConnection) < ConfigLogin.NORMAL_CONNECTION_TIME) || (currentTime - fc.lastConnection) < ConfigLogin.FAST_CONNECTION_TIME || fc.attempts > ConfigLogin.MAX_CONNECTION_PER_IP)
						{
