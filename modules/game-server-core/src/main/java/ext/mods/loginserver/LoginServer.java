...
public class LoginServer
{
	...
	public LoginServer() throws Exception
	{
		...
		ConnectionPool.init(Config.DATABASE_URL, Config.DATABASE_LOGIN, Config.DATABASE_PASSWORD, "LoginServerPool");
		...
		if (!ConfigLogin.LOGINSERVER_HOSTNAME.equals("*"))
		{
			try
			{
				bindAddress = InetAddress.getByName(ConfigLogin.LOGINSERVER_HOSTNAME);
			}
		...
		try
		{
			...
			LOGGER.info("Listening for gameservers on {}:{}.", ConfigServer.GAMESERVER_LOGIN_HOSTNAME, ConfigServer.GAMESERVER_LOGIN_PORT);
		}
		catch (IOException ioe)
		{
		...
		final int effectivePort = ConfigLogin.getEffectiveLoginServerPort();
		if (ConfigLogin.ENABLE_NATIVE_PROXY)
		{
			LOGGER.info("Reverse Proxy Mode ACTIVE: LoginServer binding on internal port {} (Public port: {}).", effectivePort, ConfigLogin.LOGINSERVER_PORT);
		}
		
		try
		{
			_selectorThread.openServerSocket(bindAddress, effectivePort);
		}
		catch (IOException ioe)
		{
		...
		LOGGER.info("Loginserver ready on {}:{}.", (bindAddress == null) ? "*" : bindAddress.getHostAddress(), effectivePort);
		ext.mods.commons.util.ShutdownSignalWatcher.startWatcher("loginserver", () -> shutdown(false));
