		...
		switch (state)
		{
			...
			case AUTHED_GG:
				...
				else if (opcode == 0xFD)
				{
					if (buf.remaining() >= 2)
					{
						final int subOpcode = buf.getShort() & 0xFFFF;
						switch (subOpcode)
						{
							case 0x0000:
								packet = new ext.mods.loginserver.network.clientpackets.RequestExternalAuthChallenge();
								break;
							case 0x0001:
								packet = new ext.mods.loginserver.network.clientpackets.RequestExternalAuthLogin();
								break;
							default:
								LOGGER.warn("Unknown Fermata Login sub-opcode: 0x" + Integer.toHexString(subOpcode));
								break;
						}
					}
				}
