...
public class ModernUI {
    ...
    /**
     * Look and feel customizado para JTabbedPane no estilo Cyberpunk Dark do ModernUI.
     */
    public static class ModernTabbedPaneUI extends javax.swing.plaf.basic.BasicTabbedPaneUI {
        @Override
        protected void installDefaults() {
            super.installDefaults();
            tabInsets = new Insets(8, 16, 8, 16);
            selectedTabPadInsets = new Insets(0, 0, 0, 0);
            tabAreaInsets = new Insets(4, 6, 2, 6);
        }
        ...
        protected void paintTabBackground(Graphics g, int tabPlacement, int tabIndex, int x, int y, int w, int h, boolean isSelected) {
            ...
            if (isSelected) {
                if (ThemeManager.isSafeGraphics()) {
                    g2.setColor(new Color(45, 30, 75));
                } else {
                    g2.setPaint(new GradientPaint(x, y, new Color(55, 30, 90), x, y + h, new Color(28, 20, 48)));
                }
                g2.fillRoundRect(x, y, w, h + 4, arc, arc);

                // Indicator line no topo ou na base da aba selecionada
                g2.setColor(NEON_CYAN);
                g2.fillRoundRect(x + 6, y + h - 2, w - 12, 3, 2, 2);
            } else {
                g2.setColor(new Color(20, 18, 28));
                g2.fillRoundRect(x, y + 2, w, h - 2, arc, arc);
            }
            g2.dispose();
        }
        ...
        protected void paintTabBorder(Graphics g, int tabPlacement, int tabIndex, int x, int y, int w, int h, boolean isSelected) {
            ...
            if (isSelected) {
                g2.setColor(new Color(140, 90, 240, 180));
                g2.setStroke(new BasicStroke(1.2f));
                g2.drawRoundRect(x, y, w - 1, h + 2, arc, arc);
            } else {
                g2.setColor(new Color(40, 35, 55, 120));
                g2.drawRoundRect(x, y + 2, w - 1, h - 4, arc, arc);
            }
            g2.dispose();
        }
        ...
        protected void paintFocusIndicator(Graphics g, int tabPlacement, Rectangle[] rects, int tabIndex, Rectangle iconRect, Rectangle textRect, boolean isSelected) {
            // Sem retângulos de foco pontilhados da era Windows 95
        }
    }
}