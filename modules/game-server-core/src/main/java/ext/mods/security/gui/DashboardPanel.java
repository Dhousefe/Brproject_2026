...
import ext.mods.security.fail2ban.gui.Fail2BanDashboard;
...
public class DashboardPanel {
    ...
    private CustomToggleSwitch tglSiteKtor;
    
    private JSlider gsSlider;
    private JLabel gsMaxLabel;
    private JSlider lsSlider;
    private JLabel lsMaxLabel;
    private volatile boolean ratesLoadStarted;
    private volatile boolean ratesLoaded;
    ...
    public DashboardPanel(JFrame parentFrame, DatabaseManager databaseManager, ProcessManagerService processManagerService, MainFrame mainFrame, IRamAllocationService ramAllocationService, IRatesManager ratesManager) {
        ...
        // 1) Cria a casca vazia do painel SEM qualquer IO. A janela aparece instantaneamente.
        createPanel();
        // 2) Inicia o monitor de RAM (timer só repinta quando os widgets ja existem).
        startRamMonitoring();
    }
    ...
    /**
     * Carrega o rates.properties em uma thread separada e, ao terminar,
     * dispara a construcao do conteudo do painel no EDT.
     * Isso evita que a GUI fique congelada esperando IO de disco.
     */
    private void scheduleRatesLoad() {
        if (ratesLoadStarted) {
            return;
        }
        ...
        Thread loader = new Thread(() -> {
            try {
                ratesManager.reloadRates();
            } catch (Throwable t) {
                System.err.println("[DashboardPanel] Falha ao carregar rates: " + t.getMessage());
            } finally {
                ratesLoaded = true;
                SwingUtilities.invokeLater(() -> updateLayout(LauncherApp.getKey()));
            }
        }, "brproject-rates-loader");
    ...
    public void updateLayout(String key) {
        ...
        if (!ratesLoaded) {
            ...
            JLabel loading = new JLabel("Carregando configurações do servidor...", JLabel.CENTER);
            loading.setForeground(ModernUI.TEXT_WHITE);
            loading.setFont(new Font("Segoe UI", Font.PLAIN, 14));
            dashboardPanel.add(loading, BorderLayout.CENTER);
        ...
        btnSiteKtorLogs.setFont(new Font("Segoe UI", Font.BOLD, 11));
        btnSiteKtorLogs.addActionListener(e -> openSiteKtorLogViewer());
        addToggleRowWithButton(toggleContainer, "Logs Módulo Ktor", btnSiteKtorLogs, 4);
    ...
    private JPanel createLeftButtons() {
    	...
    	NeonButton b4 = new NeonButton("Fail2Ban", ModernUI.NEON_PURPLE, ModernUI.NEON_CYAN, true, null);
    	...
    	b4.addActionListener(e -> { 
    		try {
    			// Initialize Fail2Ban (creates SQLite DB if needed, then opens dashboard)
    			ext.mods.security.fail2ban.core.BanManager banManager = Fail2BanInitializer.getBanManager();
    			if (banManager == null) {
    				JOptionPane.showMessageDialog(parentFrame, "Fail2Ban is disabled in proxy.xml", "Fail2Ban", JOptionPane.WARNING_MESSAGE);
    				return;
    			}
    			Fail2BanDashboard.getInstance().showWindow(banManager);
    		} catch (Exception ex) {
    			JOptionPane.showMessageDialog(parentFrame, "Fail2Ban Dashboard error: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
    		}
    	});
    ...
    private void addToggleRowWithButton(JPanel container, String label, Component btn, int y) {
        GridBagConstraints gbc = new GridBagConstraints();
        gbc.fill = GridBagConstraints.HORIZONTAL;
    ...
    private void openSiteKtorLogViewer() {
        SiteKtorLogViewerDialog.getInstance().showWindow(parentFrame);
    }

    private void toggleSiteKtor() {
        if (tglSiteKtor == null) {
            return;
        }

        if (tglSiteKtor.isOn()) {
            tglSiteKtor.setEnabled(false);
            boolean started = processManagerService.startSite(parentFrame, () -> {
                if (tglSiteKtor != null) {
                    tglSiteKtor.setEnabled(true);
                    tglSiteKtor.setOn(false);
                }
            });
            if (started) {
                tglSiteKtor.setEnabled(true);
            ...
            tglSiteKtor.setEnabled(false);
            processManagerService.stopSite();
            tglSiteKtor.setEnabled(true);
        }
    }
