...
public class hwid
{
	...
	{
		return ConfigProtection.ALLOW_GUARD_SYSTEM;
	}
	...
	public static boolean CheckHWIDs(final GameClient client, final int LastError1, final int LastError2)
	{
		...
		if (HWID.equalsIgnoreCase("fab888b1cc9de973c8046519fa841e6") && ConfigProtection.PROTECT_KICK_WITH_EMPTY_HWID)
		{
			resultHWID = true;
		}
		if (LastError1 != 0 && ConfigProtection.PROTECT_KICK_WITH_LASTERROR_HWID)
		{
	...
	{
		hwidManager.getInstance().validBox(player, ConfigProtection.PROTECT_WINDOWS_COUNT + 1, World.getInstance().getPlayers(), true);
