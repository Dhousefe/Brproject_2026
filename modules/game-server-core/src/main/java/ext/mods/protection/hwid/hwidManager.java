...
public class hwidManager
{
	...
	public boolean validBox(final Player activeChar, final Integer numberBox, final Collection<Player> world, final Boolean forcedLogOut)
	{
		if (multiboxKickTask(activeChar, numberBox, world))
		{
			if (forcedLogOut)
			{
				...
				activeChar.sendMessage("SYS: You have exceeded the PC connection limit = " + ConfigProtection.PROTECT_WINDOWS_COUNT + " box per PC.");
	...
	private static String ExcedLimit(final Player player)
	{
		...
		tb.append("<br>You have exceeded the PC connection limit.<br1>Server have limit to <font color=LEVEL>" + ConfigProtection.PROTECT_WINDOWS_COUNT + "</font> per PC.<br><br>You will be disconnected in '<font color=LEVEL>30 seconds</font>'.<br1>" + player.getName() + ", Thanks for following the server rules.<br1>Thanks.<br>");
