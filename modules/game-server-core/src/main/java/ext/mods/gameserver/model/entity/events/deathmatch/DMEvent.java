...
public class DMEvent extends Events
{
	...
	{
		NpcTemplate tmpl = NpcData.getInstance().getTemplate(ConfigEvents.DM_EVENT_PARTICIPATION_NPC_ID);
		...
		try
		{
			...
			_npcSpawn.setLoc(ConfigEvents.DM_EVENT_PARTICIPATION_NPC_COORDINATES[0], ConfigEvents.DM_EVENT_PARTICIPATION_NPC_COORDINATES[1], ConfigEvents.DM_EVENT_PARTICIPATION_NPC_COORDINATES[2], ConfigEvents.DM_EVENT_PARTICIPATION_NPC_COORDINATES[3]);
	...
	public boolean startFight()
	{
		...
		if (getPlayerCounts() < ConfigEvents.DM_EVENT_MIN_PLAYERS)
		{
			setState(EventState.INACTIVE);
			
			for (DMPlayer player : _player.values())
				player.getPlayer().addItem(ConfigEvents.DM_EVENT_PARTICIPATION_FEE[0], ConfigEvents.DM_EVENT_PARTICIPATION_FEE[1], true);
		...
		closeDoors(ConfigEvents.DM_DOORS_IDS_TO_CLOSE);
	...
	public String calculateRewards()
	{
		...
		for (int i = 0; i < ConfigEvents.DM_REWARD_FIRST_PLAYERS; i++)
		{
			if (players.isEmpty())
				break;
			...
			while (!players.isEmpty())
			{
				...
				if (!ConfigEvents.DM_REWARD_PLAYERS_TIE)
					break;
			}
		}
	...
	private void rewardPlayer(DMPlayer dmplayer, int pos)
	{
		...
		List<int[]> rewards = ConfigEvents.DM_EVENT_REWARDS.get(pos);
	...
	public void stopFight()
	{
		...
		openDoors(ConfigEvents.DM_DOORS_IDS_TO_CLOSE);
		
		String[] topPositions;
		String htmltext = "";
		if (ConfigEvents.DM_SHOW_TOP_RANK)
		{
			topPositions = getFirstPosition(ConfigEvents.DM_TOP_RANK);
			Boolean c = true;
			String c1 = "D9CC46";
			String c2 = "FFFFFF";
		...
		for (DMPlayer player : _player.values())
		{
			if (player != null)
			{
				...
				new DMEventTeleporter(player.getPlayer(), ConfigEvents.DM_EVENT_PARTICIPATION_NPC_COORDINATES, false, false);
			}
		}
	...
	{
		return player.destroyItemByItemId(ConfigEvents.DM_EVENT_PARTICIPATION_FEE[0], ConfigEvents.DM_EVENT_PARTICIPATION_FEE[1], true);
	}
	
	public String getParticipationFee()
	{
		int itemId = ConfigEvents.DM_EVENT_PARTICIPATION_FEE[0];
		int itemNum = ConfigEvents.DM_EVENT_PARTICIPATION_FEE[1];
	...
	public void onLogout(Player player)
	{
		...
		{
			if (SafeDisconnectHooks.get().isSafeDisconnectActive(player) && ConfigSafeDisconnect.SAFEDISCONNECT_INTEGRATION_DM)
				return;
			
			if (removeParticipant(player))
				player.teleportTo(ConfigEvents.DM_EVENT_PARTICIPATION_NPC_COORDINATES[0] + Rnd.get(101) - 50, ConfigEvents.DM_EVENT_PARTICIPATION_NPC_COORDINATES[1] + Rnd.get(101) - 50, ConfigEvents.DM_EVENT_PARTICIPATION_NPC_COORDINATES[2], 0);
		}
	}
	...
	public synchronized void onBypass(String command, Player player)
	{
		...
		if (command.equals("dm_event_participation"))
		{
			...
			else if (ConfigEvents.DISABLE_ID_CLASSES_DM.contains(player.getClassId().getId()))
			{
				htmContent = HTMLData.getInstance().getHtm(player, htmlPath + "Class.htm");
				if (htmContent != null)
					npcHtmlMessage.setHtml(htmContent);
			}
			else if (playerLevel < ConfigEvents.DM_EVENT_MIN_LVL || playerLevel > ConfigEvents.DM_EVENT_MAX_LVL)
			{
				htmContent = HTMLData.getInstance().getHtm(player, htmlPath + "Level.htm");
				if (htmContent != null)
				{
					npcHtmlMessage.setHtml(htmContent);
					npcHtmlMessage.replace("%min%", String.valueOf(ConfigEvents.DM_EVENT_MIN_LVL));
					npcHtmlMessage.replace("%max%", String.valueOf(ConfigEvents.DM_EVENT_MAX_LVL));
				}
			}
			else if (getPlayerCounts() == ConfigEvents.DM_EVENT_MAX_PLAYERS)
			{
				htmContent = HTMLData.getInstance().getHtm(player, htmlPath + "Full.htm");
				if (htmContent != null)
				{
					npcHtmlMessage.setHtml(htmContent);
					npcHtmlMessage.replace("%max%", String.valueOf(ConfigEvents.DM_EVENT_MAX_PLAYERS));
				}
			}
			else if ((ConfigEvents.DM_EVENT_MAX_PARTICIPANTS_PER_IP > 0) && !AntiFeedManager.getInstance().tryAddPlayer(AntiFeedManager.DM_ID, player, ConfigEvents.DM_EVENT_MAX_PARTICIPANTS_PER_IP))
			{
				htmContent = HTMLData.getInstance().getHtm(player, htmlPath + "IPRestriction.htm");
				if (htmContent != null)
				{
					npcHtmlMessage.setHtml(htmContent);
					npcHtmlMessage.replace("%max%", String.valueOf(AntiFeedManager.getInstance().getLimit(player, ConfigEvents.DM_EVENT_MAX_PARTICIPANTS_PER_IP)));
				}
			}
	...
	public boolean onScrollUse(int objectId)
	{
		...
		return isPlayerParticipant(objectId) && !ConfigEvents.DM_EVENT_SCROLL_ALLOWED;
	}
	...
	public boolean onPotionUse(int objectId)
	{
		...
		return isPlayerParticipant(objectId) && !ConfigEvents.DM_EVENT_POTIONS_ALLOWED;
	}
	...
	public boolean onItemSummon(int objectId)
	{
		...
		return isPlayerParticipant(objectId) && !ConfigEvents.DM_EVENT_SUMMON_BY_ITEM_ALLOWED;
	}
	...
	public void onTeleported(Player player)
	{
		...
		Map<Integer, Integer> buffs = player.isMageClass() ? ConfigEvents.DM_EVENT_MAGE_BUFFS : ConfigEvents.DM_EVENT_FIGHTER_BUFFS;
