...
public final class OlympiadGameTask implements Runnable
{
	...
	protected static final long BATTLE_PERIOD = ConfigEvents.OLY_BATTLE;
	...
	private volatile boolean _pausedByDisconnect = false;
		...
		try
		{
			...
			switch (_state)
			{
				case BEGIN:
					...
					_countDown = ConfigEvents.OLY_WAIT_TIME;
					break;
				...
				case BATTLE_IN_PROGRESS:
					_countDown += 1000;
					if (checkBattle() || _countDown > ConfigEvents.OLY_BATTLE)
						_state = GameState.GAME_STOPPED;
				...
				case GAME_STOPPED:
					...
					_countDown = ConfigEvents.OLY_WAIT_END;
					stopGame();
	...
	public synchronized void pauseForDisconnect(Player player)
	{
		...
		if (!_pausedByDisconnect)
		{
			...
			_zone.broadcastPacket(SystemMessage.getSystemMessage(SystemMessageId.THE_GAME_WILL_START_IN_S1_SECOND_S).addNumber(ConfigEvents.OLY_WAIT_BATTLE));
		}
	}
