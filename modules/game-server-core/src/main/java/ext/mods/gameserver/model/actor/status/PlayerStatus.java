...
public class PlayerStatus extends PlayableStatus<Player>
{
	...
	public final void reduceHp(double value, Creature attacker, boolean awake, boolean isDOT, boolean isHPConsumption, boolean ignoreCP)
	{
		...
		if (ConfigOfflineShop.OFFLINE_MODE_NO_DAMAGE && (_actor.getClient() != null) && _actor.getClient().isDetached() && ((ConfigOfflineShop.OFFLINE_TRADE_ENABLE && ((_actor.getOperateType() == OperateType.SELL) || (_actor.getOperateType() == OperateType.BUY))) || (ConfigOfflineShop.OFFLINE_CRAFT_ENABLE && (_actor.isCrafting() || (_actor.getOperateType() == OperateType.MANUFACTURE)))))
			return;
	...
	public void broadcastStatusUpdate()
	{
		...
		if (TournamentState.isIn(_actor) && TournamentState.opponents(_actor) != null && !TournamentState.opponents(_actor).isEmpty())
		{
			ExOlympiadUserInfo update = new ExOlympiadUserInfo(_actor);
			for (Player opponent : TournamentState.opponents(_actor))
			{
	...
	public boolean addExpAndSp(long addToExp, int addToSp, Map<Creature, RewardInfo> rewards)
	{
		if (_actor.hasPet())
		{
			...
			if (pet.getStatus().getExp() <= (pet.getTemplate().getPetDataEntry(81).maxExp() + 10000) && !pet.isDead() && pet.isIn3DRadius(_actor, ConfigPlayers.PARTY_RANGE))
			{
				long petExp = 0;
				int petSp = 0;
	...
	/**
	 * Base-class exp always lives on {@link PlayableStatus} fields, independent of active
	 * {@code classIndex}. Prefer this over temporarily flipping classIndex to 0.
	 */
	...
	/**
	 * Base-class level always lives on {@link PlayableStatus} fields, independent of active
	 * {@code classIndex}. Prefer this over temporarily flipping classIndex to 0.
	 */
	public final int getBaseClassLevel()
	{
		return super.getLevel();
	}
	...
	{
		double value = calcStat(Stats.REGENERATE_CP_RATE, _actor.getTemplate().getBaseCpRegen(getLevel()) * ConfigPlayers.CP_REGEN_MULTIPLIER, null, null);
	...
	/**
	 * Base-class SP always lives on {@link PlayableStatus} fields, independent of active
	 * {@code classIndex}. Prefer this over temporarily flipping classIndex to 0.
	 */
	...
	public int getCriticalHit(Creature target, L2Skill skill)
	{
		...
		return Math.min(val, ConfigProject.MAX_PCRIT_RATE);
	}
	...
	{
		return ((_actor.getRace() == ClassRace.DWARF) ? ConfigPlayers.INVENTORY_MAXIMUM_DWARF : ConfigPlayers.INVENTORY_MAXIMUM_NO_DWARF) + (int) calcStat(Stats.INV_LIM, 0, null, null);
	}
	
	public int getWareHouseLimit()
	{
		return ((_actor.getRace() == ClassRace.DWARF) ? ConfigPlayers.WAREHOUSE_SLOTS_DWARF : ConfigPlayers.WAREHOUSE_SLOTS_NO_DWARF) + (int) calcStat(Stats.WH_LIM, 0, null, null);
	}
	
	public int getPrivateSellStoreLimit()
	{
		return ((_actor.getRace() == ClassRace.DWARF) ? ConfigPlayers.MAX_PVTSTORESELL_SLOTS_DWARF : ConfigPlayers.MAX_PVTSTORESELL_SLOTS_OTHER) + (int) calcStat(Stats.P_SELL_LIM, 0, null, null);
	}
	
	public int getPrivateBuyStoreLimit()
	{
		return ((_actor.getRace() == ClassRace.DWARF) ? ConfigPlayers.MAX_PVTSTOREBUY_SLOTS_DWARF : ConfigPlayers.MAX_PVTSTOREBUY_SLOTS_OTHER) + (int) calcStat(Stats.P_BUY_LIM, 0, null, null);
	}
	
	public int getFreightLimit()
	{
		return ConfigPlayers.FREIGHT_SLOTS + (int) calcStat(Stats.FREIGHT_LIM, 0, null, null);
	}
	
	public int getDwarfRecipeLimit()
	{
		return ConfigPlayers.DWARF_RECIPE_LIMIT + (int) calcStat(Stats.REC_D_LIM, 0, null, null);
	}
	
	public int getCommonRecipeLimit()
	{
		return ConfigPlayers.COMMON_RECIPE_LIMIT + (int) calcStat(Stats.REC_C_LIM, 0, null, null);
	}
}