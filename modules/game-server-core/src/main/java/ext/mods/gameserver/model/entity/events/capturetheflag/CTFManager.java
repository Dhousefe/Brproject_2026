...
public class CTFManager
{
	...
	{
		if (ConfigEvents.CTF_EVENT_ENABLED)
		{
			if (ConfigEvents.CTF_EVENT_TEAM_1_NAME != ConfigEvents.CTF_EVENT_TEAM_2_NAME)
			{
	...
	public void scheduleEventStart()
	{
		try
		{
			...
			for (String timeOfDay : ConfigEvents.CTF_EVENT_INTERVAL)
			{
	...
	public void startReg()
	{
		...
		{
			World.announceToOnlinePlayers("CTF Event: Joinable in " + ConfigEvents.CTF_NPC_LOC_NAME + "!");
			
			if (ConfigEvents.EVENT_COMMANDS)
				World.announceToOnlinePlayers("CTF Event: Command: .ctfjoin / .ctfleave / .ctfinfo");
			
			if (ConfigEvents.ALLOW_CTF_DLG)
			{
			...
			_task.setStartTime(System.currentTimeMillis() + (60000L * ConfigEvents.CTF_EVENT_PARTICIPATION_TIME));
			ThreadPool.execute(_task);
		}
	}
	...
	public void startEvent()
	{
		...
		{
			CTFEvent.getInstance().sysMsgToAllParticipants("Teleporting in " + ConfigEvents.CTF_EVENT_START_LEAVE_TELEPORT_DELAY + " second(s).");
			_task.setStartTime(System.currentTimeMillis() + (60000L * ConfigEvents.CTF_EVENT_RUNNING_TIME));
			ThreadPool.execute(_task);
		}
	}
	...
	public void endEvent()
	{
		...
		CTFEvent.getInstance().sysMsgToAllParticipants("Teleporting back town in " + ConfigEvents.CTF_EVENT_START_LEAVE_TELEPORT_DELAY + " second(s).");
	...
	public String getTutorialAlertHtml(Player player)
	{
		...
		String message = CTFEvent.getInstance().isParticipating()
			? ("Registration is open! Join in " + ConfigEvents.CTF_NPC_LOC_NAME + ". Commands: .ctfjoin / .ctfleave / .ctfinfo")
