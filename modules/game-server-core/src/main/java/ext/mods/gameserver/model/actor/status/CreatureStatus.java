...
public class CreatureStatus<T extends Creature>
{
	...
	public double getRegenHp()
	{
		...
		return calcStat(Stats.REGENERATE_HP_RATE, _actor.getTemplate().getBaseHpRegen(getLevel()) * ConfigPlayers.HP_REGEN_MULTIPLIER * ((_actor.isChampion()) ? ConfigNpcs.CHAMPION_HP_REGEN : 1), null, null);
	}
	...
	{
		return calcStat(Stats.REGENERATE_MP_RATE, _actor.getTemplate().getBaseMpRegen(getLevel()) * ConfigPlayers.MP_REGEN_MULTIPLIER, null, null);
	}
	...
	{
		return (int) calcStat(Stats.MAGIC_ATTACK, _actor.getTemplate().getBaseMAtk() * ((_actor.isChampion()) ? ConfigNpcs.CHAMPION_MATK : 1), target, skill);
	}
	...
	{
		return (int) calcStat(Stats.MAGIC_ATTACK_SPEED, 333.0 * ((_actor.isChampion()) ? ConfigNpcs.CHAMPION_SPD_MATK : 1), null, null);
	}
	...
	{
		return (int) calcStat(Stats.POWER_ATTACK, _actor.getTemplate().getBasePAtk() * ((_actor.isChampion()) ? ConfigNpcs.CHAMPION_ATK : 1), target, null);
	}
	...
	{
		return (int) calcStat(Stats.POWER_ATTACK_SPEED, _actor.getTemplate().getBasePAtkSpd() * ((_actor.isChampion()) ? ConfigNpcs.CHAMPION_SPD_ATK : 1), null, null);
	}
