...
public class PlayerAI extends PlayableAI<Player>
{
    ...
    protected void onEvtBowAttackReuse()
    {
        ...
        {
            final Intention next = getNextIntention();
            if (next.getType() == IntentionType.ATTACK)
            {
                consumeNextIntention();
                doIntention(next);
                return;
            }
    ...
    public void thinkAttack()
    {
        ...
        if (checkObstacleAndMove(target, isShiftPressed) && ConfigGeoengine.ATTACK_USE_PATHFINDER)
            return;
		...
		if (ConfigGeoengine.SISTEMA_PATHFINDING && !MovementIntegration.canSeeTarget(_actor, target))
		{
    ...
    protected void thinkCast()
    {
        ...
        switch (skill.getTargetType())
        {
            case GROUND:
            {
                ...
                if (ConfigGeoengine.SISTEMA_PATHFINDING && groundDistance > 20.0 && groundDistance < 1000)
                {
                ...
                if (_actor.getMove().maybePlayerMoveToLocation(signetLocation, effectiveGroundRange, ConfigGeoengine.SISTEMA_PATHFINDING, isShiftPressed))
                {
            ...
            default:
            {
                if (ConfigGeoengine.SISTEMA_PATHFINDING)
                {
                    ...
                    if (distance > ConfigGeoengine.NPC_MOVEMENT_PLAYER_RANGE && distance > 20)
                    {
        ...
        {
            if (ConfigGeoengine.SISTEMA_PATHFINDING && skill.getTargetType() != SkillTargetType.GROUND 
    ...
    {
        if (isShiftPressed || !ConfigGeoengine.SISTEMA_PATHFINDING || !ConfigGeoengine.ATTACK_USE_PATHFINDER || _actor.isMovementDisabled() || !_currentIntention.canMoveToTarget())
            return false;
    ...
    protected void thinkInteract()
    {
        ...
        if (target instanceof Player && BuffShopHooks.get().isSeller(target.getObjectId()))
        {
            Player sellerNpc = (Player) target;
            Integer ownerId = BuffShopHooks.get().getSellerOwnerId(sellerNpc.getObjectId());
            ...
            if (_actor.distance3D(target) > 100)
            {
                ...
                BuffShopHooks.get().handleBypass(_actor, "showShop " + target.getObjectId() + " 1 1");
                return; 
            }
    ...
    private boolean tryShiftClick(WorldObject target, boolean isShiftPressed)
    {
        if (isShiftPressed)
        {
            ...
            else if (ConfigProject.SHOW_NPC_INFO)
            {
    ...
    private Location calculateBypassPathForTarget(Creature target)
    {
        ...
        if (ConfigGeoengine.SISTEMA_PATHFINDING)
        {
            ...
            {
                if (ConfigGeoengine.ENABLE_SMOOTH_OBSTACLE_AVOIDANCE)
                {
