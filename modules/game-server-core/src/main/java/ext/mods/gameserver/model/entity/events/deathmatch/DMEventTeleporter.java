...
public class DMEventTeleporter implements Runnable
{
	...
	{
		_player = player;
		_coordinates = ConfigEvents.DM_EVENT_PLAYER_COORDINATES.get(Rnd.get(ConfigEvents.DM_EVENT_PLAYER_COORDINATES.size()));
		_adminRemove = adminRemove;
	...
	{
		ThreadPool.schedule(this, fastSchedule ? 0 : (DMEvent.getInstance().isStarted() ? ConfigEvents.DM_EVENT_RESPAWN_TELEPORT_DELAY : ConfigEvents.DM_EVENT_START_LEAVE_TELEPORT_DELAY) * 1000);
	}
	...
	public void run()
	{
		...
		if (ConfigEvents.DM_EVENT_EFFECTS_REMOVAL == 0 || (ConfigEvents.DM_EVENT_EFFECTS_REMOVAL == 1 && (_player.getTeam() == TeamType.NONE || (_player.isInDuel() && _player.getDuelState() != DuelState.INTERRUPTED))))
