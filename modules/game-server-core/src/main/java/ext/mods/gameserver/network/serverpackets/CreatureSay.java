...
public class CreatureSay extends L2GameServerPacket
{
	...
	private final FermataLinkedItemChat _linkedItemChat;
	...
	public CreatureSay(int objectId, SayType sayType, String name, String content)
	{
		...
		_name = name;
		_content = content;
		_linkedItemChat = FermataLinkedItemChat.currentFor(content);
	}
	...
	{
		if (_linkedItemChat != null && getClient().hasFermataCapability(FermataProtocol.LINKED_ITEM_CHAT))
		{
			_linkedItemChat.writeBody(_buf, _objectId, _sayType, _name);
			return;
		}
		...
		final String content = _linkedItemChat == null ? _content : _linkedItemChat.getLegacyDisplayText();
		if (content != null)
		{
			writeS(_name);
			writeS(content);
