...
public class Party extends AbstractGroup
{
	...
	private Player getRandomValidLooter(int itemId, Creature target)
	{
		...
		for (Player member : _members)
		{
			if (!member.isDead() && member.getInventory().validateCapacityByItemId(itemId, 1) && member.isInStrictRadius(target, ConfigPlayers.PARTY_RANGE))
	...
	public void distributeAdena(Player player, int adena, Creature target)
	{
		...
		for (Player member : _members)
		{
			if (member.getAdena() == Integer.MAX_VALUE || !member.isInStrictRadius(target, ConfigPlayers.PARTY_RANGE))
				continue;
	...
	public void distributeXpAndSp(long xp_dynam, long xpReward_pr, int spReward_pr, long xpReward, int spReward, List<Player> rewardedMembers, int topLvl, Map<Creature, RewardInfo> rewards)
	{
		...
		if (ConfigPlayers.PARTY_XP_CUTOFF_METHOD.equalsIgnoreCase("level"))
		{
			for (Player member : rewardedMembers)
			{
				if (topLvl - member.getStatus().getLevel() <= ConfigPlayers.PARTY_XP_CUTOFF_LEVEL)
					validMembers.add(member);
			}
		}
		else if (ConfigPlayers.PARTY_XP_CUTOFF_METHOD.equalsIgnoreCase("percentage"))
		{
			int sqLevelSum = 0;
			for (Player member : rewardedMembers)
			...
			for (Player member : rewardedMembers)
			{
				...
				if (sqLevel * 100 >= sqLevelSum * ConfigPlayers.PARTY_XP_CUTOFF_PERCENT)
					validMembers.add(member);
			}
		}
		else if (ConfigPlayers.PARTY_XP_CUTOFF_METHOD.equalsIgnoreCase("auto"))
		{
			int sqLevelSum = 0;
			for (Player member : rewardedMembers)
				sqLevelSum += (member.getStatus().getLevel() * member.getStatus().getLevel());
		...
		else if (ConfigPlayers.PARTY_XP_CUTOFF_METHOD.equalsIgnoreCase("none"))
			validMembers.addAll(rewardedMembers);
		
		final double partyRate = BONUS_EXP_SP[Math.min(validMembers.size(), 9)];
		
		xpReward *= partyRate * ConfigRates.RATE_PARTY_XP;
		spReward *= partyRate * ConfigRates.RATE_PARTY_SP;
		xpReward_pr *= partyRate * ConfigRates.RATE_PARTY_XP;
		spReward_pr *= partyRate * ConfigRates.RATE_PARTY_SP;
		xp_dynam *= partyRate * ConfigRates.RATE_PARTY_XP;
