...
public abstract class OlympiadGameNormal extends AbstractOlympiadGame
{
	...
	protected static final Participant[] createListOfParticipants(List<Integer> list)
	{
		...
		while (list.size() > 1 && attempts++ < maxAttempts)
		{
			...
			if (ConfigProject.OLY_BLOCK_SAME_HWID && hwidManager.hasSameHwid(playerOne, playerTwo))
			{
				list.add(playerOneObjectId);
				list.add(playerTwoObjectId);
				continue;
	...
	protected void validateWinner(OlympiadStadiumZone stadium)
	{
		...
		if (_playerOne.isDefecting() || _playerTwo.isDefecting())
		{
			if (_playerOne.isDefecting())
				removePointsFromParticipant(_playerOne, Math.min(playerOnePoints / 3, ConfigEvents.OLY_MAX_POINTS));
			
			if (_playerTwo.isDefecting())
				removePointsFromParticipant(_playerTwo, Math.min(playerTwoPoints / 3, ConfigEvents.OLY_MAX_POINTS));
		...
		final int pointDiff = Math.clamp(Math.min(playerOnePoints, playerTwoPoints) / getDivider(), 1, ConfigEvents.OLY_MAX_POINTS);
		...
		else
		{
			...
			removePointsFromParticipant(_playerOne, Math.min(playerOnePoints / getDivider(), ConfigEvents.OLY_MAX_POINTS));
			removePointsFromParticipant(_playerTwo, Math.min(playerTwoPoints / getDivider(), ConfigEvents.OLY_MAX_POINTS));
		}
