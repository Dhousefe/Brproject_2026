...
public class FishingChampionshipManager
{
	...
	{
		if (!ConfigEvents.ALLOW_FISH_CHAMPIONSHIP)
			return;
			...
			if (fisher.getRewardType() != 2 && fisher.getName().equalsIgnoreCase(player.getName()))
			{
				...
				for (int x = 0; x < _winPlayersName.size(); x++)
				{
					if (_winPlayersName.get(x).equalsIgnoreCase(player.getName()))
					{
						...
						{
							case 0:
								rewardCnt = ConfigEvents.FISH_CHAMPIONSHIP_REWARD_1;
								break;
							
							case 1:
								rewardCnt = ConfigEvents.FISH_CHAMPIONSHIP_REWARD_2;
								break;
							
							case 2:
								rewardCnt = ConfigEvents.FISH_CHAMPIONSHIP_REWARD_3;
								break;
							
							case 3:
								rewardCnt = ConfigEvents.FISH_CHAMPIONSHIP_REWARD_4;
								break;
							
							case 4:
								rewardCnt = ConfigEvents.FISH_CHAMPIONSHIP_REWARD_5;
								break;
						}
					}
				}
				...
				if (rewardCnt > 0)
				{
					player.addItem(ConfigEvents.FISH_CHAMPIONSHIP_REWARD_ITEM, rewardCnt, true);
	...
	public void showMidResult(Player player)
	{
		...
		html.replace("%prizeItem%", ItemData.getInstance().getTemplate(ConfigEvents.FISH_CHAMPIONSHIP_REWARD_ITEM).getName());
		html.replace("%prizeFirst%", ConfigEvents.FISH_CHAMPIONSHIP_REWARD_1);
		html.replace("%prizeTwo%", ConfigEvents.FISH_CHAMPIONSHIP_REWARD_2);
		html.replace("%prizeThree%", ConfigEvents.FISH_CHAMPIONSHIP_REWARD_3);
		html.replace("%prizeFour%", ConfigEvents.FISH_CHAMPIONSHIP_REWARD_4);
		html.replace("%prizeFive%", ConfigEvents.FISH_CHAMPIONSHIP_REWARD_5);
		player.sendPacket(html);
	}
