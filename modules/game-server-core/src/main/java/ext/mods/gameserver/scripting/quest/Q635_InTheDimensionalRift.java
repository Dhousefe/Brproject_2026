...
public class Q635_InTheDimensionalRift extends Quest
{
	...
	{
		ConfigEvents.RIFT_ENTER_COST_RECRUIT,
		ConfigEvents.RIFT_ENTER_COST_SOLDIER,
		ConfigEvents.RIFT_ENTER_COST_OFFICER,
		ConfigEvents.RIFT_ENTER_COST_CAPTAIN,
		ConfigEvents.RIFT_ENTER_COST_COMMANDER,
		ConfigEvents.RIFT_ENTER_COST_HERO
	};
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		else if (event.equalsIgnoreCase("rift_watcher_05.htm"))
		{
			...
			if (party0 == null || party0.getMembersCount() < ConfigEvents.RIFT_MIN_PARTY_SIZE)
			...
			if (rlist0 != null)
			{
				...
				if (i3 != -2 && i5 >= 3)
				{
					...
					for (int i4 = 0; i4 < (ConfigProject.ENTER_ANAKAZEL ? 9 : 8); i4++)
					{
						if (i3 == -1)
						{
							i5 = Rnd.get(ConfigProject.ENTER_ANAKAZEL ? 9 : 8);
							room0 = rlist0.get(i5);
		...
		else if (event.equalsIgnoreCase("ChangeRiftRoom"))
		{
			...
			final List<RoomInfo> rlist1 = rlist0.stream().filter(r -> !r.isLocked() && (ConfigProject.ENTER_ANAKAZEL ? true : r.getIndex() != 8)).toList();
		...
		if (name.equalsIgnoreCase("2001"))
		{
			if (room0 != null)
			{
				...
				if (room0.isLocked())
				{
					...
					{
						if (gtTicks - room0.getTime() > (60 * ConfigEvents.RIFT_AUTO_JUMPS_TIME_MIN + Rnd.get(ConfigEvents.RIFT_AUTO_JUMPS_TIME_RND) * 30))
						{
							if (npc._i_ai2 == 1)
							{
		...
		else if (name.equalsIgnoreCase("2002"))
		{
			...
			else
			{
				...
				else
				{
					...
					if (roomIndex != 8 && Rnd.get(100) < ConfigEvents.RIFT_ANAKAZEL_PORT_CHANCE && !room1.isLocked())
					{
