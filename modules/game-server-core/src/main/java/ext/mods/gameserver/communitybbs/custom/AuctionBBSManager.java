...
public class AuctionBBSManager extends BaseBBSManager
{
	...
	public void sendIndexMine(Player player, int page, Function function)
	{
		...
		else
		{
			...
			auctionTemplate = auctionTemplate.replace("%page%", String.valueOf(page));
			auctionTemplate = auctionTemplate.replace("%auctionFee%", StringUtil.formatNumber(ConfigProject.AUCTION_FEE));
			auctionTemplate = auctionTemplate.replace("%auctionFeeName%", ConfigProject.AUCTION_ITEM_FEE_NAME);
			listBuilder.append(auctionTemplate);
			content = content.replace("%template%", list.getContent() + listBuilder.toString());
		}
	...
	public String getInventory(Player player, int page)
	{
		...
		if (item != null)
		{
			...
			content = content.replace("%auction_fee%", StringUtil.formatNumber(ConfigProject.AUCTION_FEE));
			content = content.replace("%auctionFeeName%", ConfigProject.AUCTION_ITEM_FEE_NAME);
	...
	public boolean selectItem(Player player, ItemInstance item)
	{
		...
		if (_auctions.values().stream().filter(a -> a.getObjectId() == player.getObjectId()).count() >= ConfigProject.AUCTION_LIMIT_ITEM)
		{
			player.sendMessage(player.getSysString(10_204, ConfigProject.AUCTION_LIMIT_ITEM));
			return true;
		}
	...
	public void sellItem(Player player, int itemId, int quantity, String currency, int price)
	{
		...
		if (!player.destroyItemByItemId(ConfigProject.AUCTION_ITEM_FEE, ConfigProject.AUCTION_FEE, true))
		{
			player.sendMessage(player.getSysString(10_209, StringUtil.formatNumber(ConfigProject.AUCTION_FEE), ConfigProject.AUCTION_ITEM_FEE_NAME));
			return;
		}
	...
	{
		if (ConfigProject.ENABLED_AUCTION)
