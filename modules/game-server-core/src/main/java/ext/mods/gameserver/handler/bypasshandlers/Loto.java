...
public class Loto implements IBypassHandler {
	...
	public void showLotoWindow(Player player, int val, Npc npc)
	{
		...
		else if (val == 22)
		{
			...
			if (!player.reduceAdena(ConfigEvents.LOTTERY_TICKET_PRICE, true))
				return;
			
			LotteryManager.getInstance().increasePrize(ConfigEvents.LOTTERY_TICKET_PRICE);
		...
		else if (val == 25)
		{
			...
			html.replace("%prize5%", ConfigEvents.LOTTERY_5_NUMBER_RATE * 100);
			html.replace("%prize4%", ConfigEvents.LOTTERY_4_NUMBER_RATE * 100);
			html.replace("%prize3%", ConfigEvents.LOTTERY_3_NUMBER_RATE * 100);
			html.replace("%prize2%", ConfigEvents.LOTTERY_2_AND_1_NUMBER_PRIZE);
		}
		else if (val > 25)
		{
		...
		html.replace("%ticket_price%", ConfigEvents.LOTTERY_TICKET_PRICE);
