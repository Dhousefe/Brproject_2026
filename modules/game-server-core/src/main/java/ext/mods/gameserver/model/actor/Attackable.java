...
public class Attackable extends Npc
{
	...
	public boolean doDie(Creature killer)
	{
		...
		try
		{
			...
			if (player != null)
			{
				...
				FarmEventHooks.get().onKill(player, this);
				
				setQuestKiller(player);
				if (BattleBossState.isInEvent(player))
				{
					BattleBossHooks.get().onPlayerDeathMonster(this, player);
				}
			}
		}
		catch (Exception e)
	...
	public void setQuestKiller(Player player)
	{
		...
		for (int questId : QuestKillState.activeQuestIds(player))
		{
			...
			QuestKillState.increase(player, questId, getNpcId());
			
			boolean allRequirementsMet = true;
			for (QuestObjective obj : objectives)
			{
				long currentKills = QuestKillState.getKillCount(player, questId, obj.getNpcId());
			...
			if (allRequirementsMet)
			{
				...
				QuestKillState.setCompleted(player, questId, true);
	...
	{
		return ConfigNpcs.MAX_DRIFT_RANGE;
	}
