		...
		switch (command)
		{
			...
			case "sellbuffchangeprice":
				if (!player.isSellingBuffs() && (params != null) && !params.isEmpty())
				{
					...
					if (st.hasMoreTokens())
					{
						...
						{
							player.sendMessage("Too big price! Maximum price is " + ConfigProject.SELLBUFF_MAX_PRICE);
			...
			case "sellbuffaddskill":
				if (!player.isSellingBuffs() && params != null && !params.isEmpty())
				{
					...
					if (price > ConfigProject.SELLBUFF_MAX_PRICE)
					{
						player.sendMessage("Too big price! Maximum price is " + ConfigProject.SELLBUFF_MAX_PRICE);
						SellBuffsManager.getInstance().sendBuffChoiceMenu(player, index);
						return false;
					...
					if (price < ConfigProject.SELLBUFF_MIN_PRICE)
					{
						player.sendMessage("Too small price! Minimum price is " + ConfigProject.SELLBUFF_MIN_PRICE);
						SellBuffsManager.getInstance().sendBuffChoiceMenu(player, index);
						return false;
					}
					
					if (player.getSellingBuffs().size() >= ConfigProject.SELLBUFF_MAX_BUFFS)
					{
						player.sendMessage("You already reached max count of buffs! Max buffs is: " + ConfigProject.SELLBUFF_MAX_BUFFS);
						SellBuffsManager.getInstance().sendBuffChoiceMenu(player, index);
						return false;
					}
			...
			case "sellbuffbuyskill":
				if (params != null && !params.isEmpty())
				{
					...
					if (seller.getStatus().getMp() < (Math.max(skillToBuy.getMpConsume(), 50) * ConfigProject.SELLBUFF_MP_MULTIPLER))
					{
					...
					if (player.getInventory().getItemCount(ConfigProject.SELLBUFF_PAYMENT_ID) >= holder.getPrice())
					{
						...
						player.destroyItemByItemId(ConfigProject.SELLBUFF_PAYMENT_ID, holder.getPrice(), true);
						seller.addItem(ConfigProject.SELLBUFF_PAYMENT_ID, holder.getPrice(), true);
						
						seller.getStatus().reduceMp(Math.max(skillToBuy.getMpConsume(), 50) * ConfigProject.SELLBUFF_MP_MULTIPLER);
						...
						if (ConfigProject.CUSTOM_TIME_BUFF)
						{
					...
					{
						final var item = ItemData.getInstance().getTemplate(ConfigProject.SELLBUFF_PAYMENT_ID);
						if (item != null)
