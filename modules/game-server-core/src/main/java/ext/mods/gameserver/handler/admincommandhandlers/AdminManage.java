...
public class AdminManage implements IAdminCommandHandler
{
	...
	private static boolean kill(Creature creature, Player player)
	{
		...
		creature.stopAllEffects();
		creature.reduceCurrentHp(creature.isChampion() ? creature.getStatus().getMaxHp() * ConfigNpcs.CHAMPION_HP + 1 : creature.getStatus().getMaxHp() + creature.getStatus().getMaxCp() + 1, player, null);
		return true;
	}
