...
public class WarehouseKeeper extends Folk
{
	...
	{
		if (!ConfigPlayers.KARMA_PLAYER_CAN_USE_WH && player.getKarma() > 0 && showPkDenyChatWindow(player, "warehouse"))
			return;
		...
		else if (command.startsWith("WithdrawF"))
		{
			...
			freight.setActiveLocation((ConfigPlayers.REGION_BASED_FREIGHT) ? getRegion().hashCode() : 0);
