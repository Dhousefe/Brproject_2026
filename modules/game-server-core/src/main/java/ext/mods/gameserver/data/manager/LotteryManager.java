...
public class LotteryManager
{
	...
	{
		_number = 1;
		_prize = ConfigEvents.LOTTERY_PRIZE;
		_isSellingTickets = false;
		_isStarted = false;
		_endDate = System.currentTimeMillis();
		
		if (ConfigServer.ALLOW_LOTTERY)
			...
			try (ResultSet rs = ps.executeQuery())
			{
				if (rs.next())
				{
					...
					switch (count)
					{
						...
						default:
							res[0] = 4;
							res[1] = ConfigEvents.LOTTERY_2_AND_1_NUMBER_PRIZE;
					}
				}
			}
		}
	...
	private class FinishLottery implements Runnable
	{
		...
		public void run()
		{
			...
			int prize4 = count4 * ConfigEvents.LOTTERY_2_AND_1_NUMBER_PRIZE;
			int prize1 = 0;
			int prize2 = 0;
			int prize3 = 0;
			
			if (count1 > 0)
				prize1 = (int) ((getPrize() - prize4) * ConfigEvents.LOTTERY_5_NUMBER_RATE / count1);
			
			if (count2 > 0)
				prize2 = (int) ((getPrize() - prize4) * ConfigEvents.LOTTERY_4_NUMBER_RATE / count2);
			
			if (count3 > 0)
				prize3 = (int) ((getPrize() - prize4) * ConfigEvents.LOTTERY_3_NUMBER_RATE / count3);
			
			int newPrize = ConfigEvents.LOTTERY_PRIZE + getPrize() - (prize1 + prize2 + prize3 + prize4);
