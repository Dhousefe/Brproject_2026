...
public class TvTManager
{
	...
	{
		if (ConfigEvents.TVT_EVENT_ENABLED)
		{
	...
	public void scheduleEventStart()
	{
		try
		{
			...
			for (String timeOfDay : ConfigEvents.TVT_EVENT_INTERVAL)
			{
	...
	public void startReg()
	{
		...
		{
			World.announceToOnlinePlayers("Team vs Team: Joinable in " + ConfigEvents.TVT_NPC_LOC_NAME + "!");
			
			if (ConfigEvents.EVENT_COMMANDS)
				World.announceToOnlinePlayers("Team vs Team: Command: .tvtjoin / .tvtleave / .tvtinfo");
			
			if (ConfigEvents.ALLOW_TVT_DLG)
			{
			...
			_task.setStartTime(System.currentTimeMillis() + (60000L * ConfigEvents.TVT_EVENT_PARTICIPATION_TIME));
			ThreadPool.execute(_task);
		}
	}
	...
	public void startEvent()
	{
		...
		{
			TvTEvent.getInstance().sysMsgToAllParticipants("Teleporting participants in " + ConfigEvents.TVT_EVENT_START_LEAVE_TELEPORT_DELAY + " second(s).");
			_task.setStartTime(System.currentTimeMillis() + (60000L * ConfigEvents.TVT_EVENT_RUNNING_TIME));
			ThreadPool.execute(_task);
		}
	}
	...
	public void endEvent()
	{
		...
		TvTEvent.getInstance().sysMsgToAllParticipants("Teleporting back to town in " + ConfigEvents.TVT_EVENT_START_LEAVE_TELEPORT_DELAY + " second(s).");
	...
	public String getTutorialAlertHtml(Player player)
	{
		...
		String message = TvTEvent.getInstance().isParticipating()
			? ("Registration is open! Join in " + ConfigEvents.TVT_NPC_LOC_NAME + ". Commands: .tvtjoin / .tvtleave / .tvtinfo")
