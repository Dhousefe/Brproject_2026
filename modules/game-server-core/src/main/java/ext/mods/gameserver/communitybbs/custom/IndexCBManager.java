...
public class IndexCBManager extends BaseBBSManager
{
	...
	public void parseCmd(String command, Player player)
	{
		if (command.startsWith("_bbshome"))
		{
			...
			{
				content = content.replace("%rate_xp%", String.valueOf(getRateNumber(ConfigRates.PREMIUM_RATE_XP)));
				content = content.replace("%rate_sp%", String.valueOf(getRateNumber(ConfigRates.PREMIUM_RATE_SP)));
				content = content.replace("%rate_adena%", String.valueOf(getRateNumber(ConfigRates.PREMIUM_RATE_DROP_CURRENCY)));
				content = content.replace("%rate_seal%", String.valueOf(getRateNumber(ConfigRates.PREMIUM_RATE_DROP_SEAL_STONE)));
				content = content.replace("%rate_items%", String.valueOf(getRateNumber(ConfigRates.PREMIUM_RATE_DROP_ITEMS)));
				content = content.replace("%rate_spoil%", String.valueOf(getRateNumber(ConfigRates.PREMIUM_RATE_DROP_SPOIL)));
				content = content.replace("%rate_quest%", String.valueOf(getRateNumber(ConfigRates.PREMIUM_RATE_QUEST_DROP)));
				content = content.replace("%rate_raid%", String.valueOf(getRateNumber(ConfigRates.PREMIUM_RATE_DROP_ITEMS_BY_RAID)));
			}
			else
			{
				content = content.replace("%rate_xp%", String.valueOf(getRateNumber(ConfigRates.RATE_XP)));
				content = content.replace("%rate_sp%", String.valueOf(getRateNumber(ConfigRates.RATE_SP)));
				content = content.replace("%rate_adena%", String.valueOf(getRateNumber(ConfigRates.RATE_DROP_CURRENCY)));
				content = content.replace("%rate_seal%", String.valueOf(getRateNumber(ConfigRates.RATE_DROP_SEAL_STONE)));
				content = content.replace("%rate_items%", String.valueOf(getRateNumber(ConfigRates.RATE_DROP_ITEMS)));
				content = content.replace("%rate_spoil%", String.valueOf(getRateNumber(ConfigRates.RATE_DROP_SPOIL)));
				content = content.replace("%rate_quest%", String.valueOf(getRateNumber(ConfigRates.RATE_QUEST_DROP)));
				content = content.replace("%rate_raid%", String.valueOf(getRateNumber(ConfigRates.RATE_DROP_ITEMS_BY_RAID)));
			}
			...
			if (player.getPremiumService() == 0)
			{
				...
				content = content.replace("%ip%", player.getClient() != null ? player.getClient().getIpAddress() : "127.0.0.1");
				...
				content = content.replace("%server_rate_sp%", String.valueOf(getRateNumber(ConfigRates.RATE_SP)));
				content = content.replace("%server_rate_adena%", String.valueOf(getRateNumber(ConfigRates.RATE_DROP_CURRENCY)));
				content = content.replace("%server_rate_items%", String.valueOf(getRateNumber(ConfigRates.RATE_DROP_ITEMS)));
				content = content.replace("%server_rate_spoil%", String.valueOf(getRateNumber(ConfigRates.RATE_DROP_SPOIL)));
				content = content.replace("%server_rate_quest%", String.valueOf(getRateNumber(ConfigRates.RATE_QUEST_DROP)));
				content = content.replace("%server_quest_reward%", String.valueOf(getRateNumber(ConfigRates.RATE_QUEST_REWARD)));
				content = content.replace("%server_r_drop%", String.valueOf(getRateNumber(ConfigRates.RATE_DROP_ITEMS_BY_RAID)));
				content = content.replace("%server_g_drop%", String.valueOf(getRateNumber(ConfigRates.RATE_DROP_ITEMS_BY_GRAND)));
				content = content.replace("%server_karma_drop%", String.valueOf(getRateNumber(ConfigRates.KARMA_RATE_DROP)));
			}
			else
			{
				content = content.replace("%nameaccount%", player.getAccountName());
				...
				content = content.replace("%server_rate_sp%", String.valueOf(getRateNumber(ConfigRates.PREMIUM_RATE_SP)));
				content = content.replace("%server_rate_adena%", String.valueOf(getRateNumber(ConfigRates.PREMIUM_RATE_DROP_CURRENCY)));
				content = content.replace("%server_rate_items%", String.valueOf(getRateNumber(ConfigRates.PREMIUM_RATE_DROP_ITEMS)));
				content = content.replace("%server_rate_spoil%", String.valueOf(getRateNumber(ConfigRates.PREMIUM_RATE_DROP_SPOIL)));
				content = content.replace("%server_rate_quest%", String.valueOf(getRateNumber(ConfigRates.PREMIUM_RATE_QUEST_DROP)));
				content = content.replace("%server_quest_reward%", String.valueOf(getRateNumber(ConfigRates.PREMIUM_RATE_QUEST_REWARD)));
				content = content.replace("%server_r_drop%", String.valueOf(getRateNumber(ConfigRates.RATE_DROP_ITEMS_BY_RAID)));
				content = content.replace("%server_g_drop%", String.valueOf(getRateNumber(ConfigRates.RATE_DROP_ITEMS_BY_GRAND)));
				content = content.replace("%server_karma_drop%", String.valueOf(getRateNumber(ConfigRates.KARMA_RATE_DROP)));
			}
			
			content = content.replace("%rate_party_xp%", String.valueOf(getRateNumber(ConfigRates.RATE_PARTY_XP)));
			content = content.replace("%rate_party_sp%", String.valueOf(getRateNumber(ConfigRates.RATE_PARTY_SP)));
			content = content.replace("%rate_drop_manor%", String.valueOf(getRateNumber(ConfigRates.RATE_DROP_MANOR)));
			content = content.replace("%pet_rate_xp%", String.valueOf(getRateNumber(ConfigRates.PET_XP_RATE)));
			content = content.replace("%sineater_rate_xp%", String.valueOf(getRateNumber(ConfigRates.SINEATER_XP_RATE)));
			content = content.replace("%pet_food_rate%", String.valueOf(getRateNumber(ConfigRates.PET_FOOD_RATE)));
			content = content.replace("%server_stone_top%", String.valueOf(getRateNumber(ConfigPlayers.AUGMENTATION_TOP_SKILL_CHANCE)));
			content = content.replace("%server_stone_high%", String.valueOf(getRateNumber(ConfigPlayers.AUGMENTATION_HIGH_SKILL_CHANCE)));
			content = content.replace("%server_stone_mid%", String.valueOf(getRateNumber(ConfigPlayers.AUGMENTATION_MID_SKILL_CHANCE)));
			content = content.replace("%server_stone_low%", String.valueOf(getRateNumber(ConfigPlayers.AUGMENTATION_NG_SKILL_CHANCE)));
			content = content.replace("%deathpenality%", String.valueOf(getRateNumber(ConfigPlayers.DEATH_PENALTY_CHANCE)));
