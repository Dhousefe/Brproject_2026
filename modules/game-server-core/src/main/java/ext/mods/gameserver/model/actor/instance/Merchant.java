...
public class Merchant extends Folk
{
	...
	{
		if (!ConfigPlayers.KARMA_PLAYER_CAN_SHOP && getNpcId() != 30420 && player.getKarma() > 0 && showPkDenyChatWindow(player, "merchant"))
			return;
		...
		else if (actualCommand.equalsIgnoreCase("Wear") && ConfigServer.ALLOW_WEAR)
		{
			if (st.countTokens() < 1)
				return;
		...
		else if (actualCommand.equalsIgnoreCase("Multisell_Shadow"))
		{
			...
			if (!ConfigServer.ALLOW_SHADOW_WEAPONS)
			{
