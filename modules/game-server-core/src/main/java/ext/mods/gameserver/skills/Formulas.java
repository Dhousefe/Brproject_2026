...
public final class Formulas
{
	...
	public static final double calcMagicDam(Creature attacker, Creature target, L2Skill skill, ShieldDefense sDef, boolean ss, boolean bss, boolean mcrit)
	{
		...
		if (ConfigPlayers.MAGIC_FAILURES && !calcMagicSuccess(attacker, target, skill))
		{
	...
	public static final double calcMagicDam(Cubic attacker, Creature target, L2Skill skill, boolean mcrit, ShieldDefense sDef)
	{
		...
		if (ConfigPlayers.MAGIC_FAILURES && !calcMagicSuccess(owner, target, skill))
		{
	...
	/**
	 * Calculate the post-hit attack recovery factor based on DEX.
	 * High-DEX characters (Daggers, Archers) recover up to 35-50% faster,
	 * enabling rapid target switching and fluid stutter-step movement.
	 * Low-DEX characters (Mages) retain deliberate physical recovery lock.
	 *
	 * @param attacker The {@link Creature} performing the attack.
	 * @return A double multiplier to scale the recovery delay (1.0 = standard, < 1.0 = faster).
	 */
	public static final double calcDexAttackRecoveryFactor(Creature attacker)
	{
		if (!ConfigPlayers.ENABLE_DEX_ATTACK_RECOVERY_SCALING || !(attacker instanceof Player))
			return 1.0;

		final int dex = attacker.getStatus().getDEX();
		final double scaled = 1.0 - (((double) (dex - ConfigPlayers.DEX_ATTACK_RECOVERY_BASE_DEX) / ConfigPlayers.DEX_ATTACK_RECOVERY_DEX_DIVISOR) * ConfigPlayers.DEX_ATTACK_RECOVERY_MAX_BONUS);
		return Math.clamp(scaled, ConfigPlayers.DEX_ATTACK_RECOVERY_MIN_FACTOR, ConfigPlayers.DEX_ATTACK_RECOVERY_MAX_FACTOR);
	}
	...
	public static ShieldDefense calcShldUse(Creature attacker, Creature target, L2Skill skill, boolean isCrit)
	{
		...
		ShieldDefense sDef;
		if (chance < ConfigPlayers.PERFECT_SHIELD_BLOCK_RATE)
	...
	{
		if (!ConfigServer.ENABLE_FALLING_DAMAGE || safeFallHeight < 0)
			return 0;
