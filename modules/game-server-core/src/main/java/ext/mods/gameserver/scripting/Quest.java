...
public class Quest
{
	...
	/**
	 * Check a {@link Player}'s {@link QuestState} condition. {@link Player} must be within ConfigPlayers.PARTY_RANGE distance from the {@link Npc}. If {@link Npc} is null, distance condition is ignored.
	...
	public QuestState checkPlayerCondition(Player player, Npc npc, int cond)
	{
		...
		if (!player.isIn3DRadius(npc, ConfigPlayers.PARTY_RANGE))
			return null;
	...
	/**
	 * Check a {@link Player}'s {@link Clan} leader {@link QuestState} condition. Both of them must be within ConfigPlayers.PARTY_RANGE distance from the {@link Npc}. If {@link Npc} is null, distance condition is ignored.
	 * @param player : The {@link Player} who acted towards the {@link Npc}.
	...
	/**
	 * Check the {@link Player}'s {@link QuestState} state. {@link Player} must be within ConfigPlayers.PARTY_RANGE distance from the {@link Npc}.
	 * @param player : The {@link Player} who acted towards the {@link Npc}.
	 * @param npc : The {@link Npc} used to compare distance.
	...
	/**
	 * Check a {@link Player}'s {@link Clan} leader {@link QuestState} state. Both of them must be within ConfigPlayers.PARTY_RANGE distance from the {@link Npc}. If {@link Npc} is null, distance condition is ignored.
	 * @param player : The {@link Player} who acted towards the {@link Npc}.
	 * @param npc : The {@link Npc} used to compare distance (optional).
	...
	public QuestState getClanLeaderQuestState(Player player, Npc npc)
	{
		...
		if (npc != null && !player.isIn3DRadius(npc, ConfigPlayers.PARTY_RANGE))
			return null;
		...
		if (npc != null && !leader.isIn3DRadius(npc, ConfigPlayers.PARTY_RANGE))
			return null;
	...
	public Npc addSpawn(int npcId, int x, int y, int z, int heading, boolean randomOffset, long despawnDelay, boolean isSummonSpawn)
	{
		try
		{
			...
			if (randomOffset)
			{
				...
				z = GeoEngine.getInstance().getHeight(x, y, z - ConfigGeoengine.NPC_Z_INDEX);
	...
	/**
	 * Drop items to the {@link Player}'s inventory. Rate is 100%, amount is affected by ConfigRates.RATE_QUEST_DROP.
	...
	/**
	 * Drop items to the {@link Player}'s inventory. Rate and amount is affected by DIVMOD of ConfigRates.RATE_QUEST_DROP.
	 * @param player : The {@link Player} to drop items.
	 * @param itemId : Identifier of the item to be dropped.
	 * @param count : Quantity of items to be dropped.
	 * @param neededCount : Quantity of items needed to complete the task. If set to 0, unlimited amount is collected.
	...
	public static boolean dropItems(Player player, int itemId, int count, int neededCount, int dropChance, byte type)
	{
		...
		{
			case DROP_DIVMOD:
				dropChance *= player.getPremiumService() == 1 ? ConfigRates.PREMIUM_RATE_QUEST_DROP : ConfigRates.RATE_QUEST_DROP;
			...
			case DROP_FIXED_RATE:
				if (Rnd.get(DropData.MAX_CHANCE) < dropChance)
					amount = (int) (count * (player.getPremiumService() == 1 ? ConfigRates.PREMIUM_RATE_QUEST_DROP : ConfigRates.RATE_QUEST_DROP));
				break;
			
			case DROP_FIXED_COUNT:
				if (Rnd.get(DropData.MAX_CHANCE) < dropChance * (player.getPremiumService() == 1 ? ConfigRates.PREMIUM_RATE_QUEST_DROP : ConfigRates.RATE_QUEST_DROP))
					amount = count;
				break;
	...
	/**
	 * Drop multiple items to the {@link Player}'s inventory. Rate and amount is affected by DIVMOD of ConfigRates.RATE_QUEST_DROP.
	 * @param player : The {@link Player} to drop items.
	...
	/**
	 * Reward {@link Player} with items. The amount is affected by ConfigRates.RATE_QUEST_REWARD or ConfigRates.RATE_QUEST_REWARD_ADENA.
	...
	public static void rewardItems(Player player, int itemId, int itemCount)
	{
		if (itemId == 57)
		{
			double reward = (itemCount * (player.getPremiumService() == 1 ? ConfigRates.PREMIUM_RATE_QUEST_REWARD_ADENA : ConfigRates.RATE_QUEST_REWARD_ADENA));
			giveItems(player, itemId, (int) reward, 0);
		}
		else
		{
			double reward = (itemCount * (player.getPremiumService() == 1 ? ConfigRates.PREMIUM_RATE_QUEST_REWARD : ConfigRates.RATE_QUEST_REWARD));
			giveItems(player, itemId, (int) reward, 0);
		}
	}
	...
	/**
	 * Reward {@link Player} with EXP and SP. The amount is affected by ConfigRates.RATE_QUEST_REWARD_XP and ConfigRates.RATE_QUEST_REWARD_SP.
	...
	{
		player.addExpAndSp((long) (exp * (player.getPremiumService() == 1 ? ConfigRates.PREMIUM_RATE_QUEST_REWARD_XP : ConfigRates.RATE_QUEST_REWARD_XP)), (int) (sp * (player.getPremiumService() == 1 ? ConfigRates.PREMIUM_RATE_QUEST_REWARD_SP : ConfigRates.RATE_QUEST_REWARD_SP)));
	}
	...
	public final String getHtmlText(String fileName, Player player)
	{
		if (isRealQuest())
		{
			final String questPath = "html/script/quest/" + getName() + "/" + fileName;
			
			if (player != null && TranslatorState.isHtmlTranslationEnabled(player))
			{
				final Locale playerLocale = player.getLocale();
				if (playerLocale != null && !playerLocale.equals(ConfigLanguage.DEFAULT_LOCALE) && HTMLData.getInstance().exists(playerLocale, questPath))
					return HTMLData.getInstance().getHtm(playerLocale, questPath);
				
				if (TranslatorHooks.get().isTranslatorAvailable())
				{
					final String translated = TranslatorHooks.get().translateFile(player, questPath);
					if (translated != null)
					{
						if (HTMLData.getInstance().exists(playerLocale, questPath))
							return HTMLData.getInstance().getHtm(playerLocale, questPath);
						if (!translated.isEmpty() && translated.indexOf('<') >= 0)
							return translated;
					}
				}
			}
			
			return HTMLData.getInstance().getHtm(player != null ? player.getLocale() : ConfigLanguage.DEFAULT_LOCALE, questPath);
		}
	...
	/**
	 * Translate text via TranslatorHooks (NOOP when crypta/deepl mod is absent).
	 */
	protected String translateText(Player player, String text)
	{
		if (player == null || text == null || !TranslatorState.isHtmlTranslationEnabled(player))
			return text;
		
		return TranslatorHooks.get().translateText(player, text);
	}
