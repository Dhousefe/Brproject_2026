...
public class NpcAI<T extends Npc> extends CreatureAI<T>
{
    ...
    protected final IntSet _seenCreatures = new IntOpenHashSet(8, Hash.FAST_LOAD_FACTOR);
    ...
    public void thinkAttack() {
        ...
        if (!_actor.knows(target)) {
            if (_actor instanceof Npc npc) {
                ...
                if (_actor.isIn3DRadius(target, aggroRange + 100)) {
                    ...
                    if (ConfigNpcs.DEBUG_MELEE_ATTACK && weaponRange <= 200) {
        ...
        // Tolerancia canonica retail / aCis 409: moving target compensacao
        int movingTolerance = target.isMoving() ? 50 : 15;
        double maxAttackRange = totalRange + movingTolerance;
        ...
        if (weaponRange <= 200 && !_actor.getAllSkillsDisabled() && !_actor.getCast().isCastingNow() && Rnd.get(100) < 30) {
            ...
            if (skill != null) {
                ...
                if (ConfigNpcs.DEBUG_MELEE_ATTACK) {
    ...
    public void runAI(boolean updateTick)
    {
        ...
        if (updateTick)
        {
            ...
            if (!scripts.isEmpty()) {
                // UC1: Engaged mob bypass
                // If a non-boss monster is actively attacking a living target and has active aggro,
                // skip the heavy scanning of surrounding Playable entities.
                final boolean isEngagedInAttack = !_actor.isRaidBoss()
                    && _currentIntention.getType() == IntentionType.ATTACK
                    && _currentIntention.getFinalTarget() != null
                    && !_currentIntention.getFinalTarget().isAlikeDead()
                    && !_aggroList.isEmpty();

                // Pacing for idle monsters: distribute non-boss perception over phases
                final boolean shouldScanPerception = !isEngagedInAttack
                    && (_actor.isRaidBoss() || _lifeTime == 0 || ((_actor.getObjectId() + _runAiStep) & 1) == 0);

                if (shouldScanPerception) {
                    final int seeRange = _actor.getSeeRange();
                    final int seeRangeSq = seeRange * seeRange;
                    final int actorX = _actor.getX();
                    final int actorY = _actor.getY();
                    final int actorZ = _actor.getZ();

                    _actor.forEachKnownType(Playable.class, pl -> {
                        if (pl == null || pl.isAlikeDead()) {
                            _seenCreatures.remove(pl.getObjectId());
                            return;
                        }

                        // UC2: Fast AABB and squared distance reject before any heavy checks
                        final int dx = pl.getX() - actorX;
                        final int dy = pl.getY() - actorY;
                        final int dz = pl.getZ() - actorZ;

                        if (Math.abs(dx) > seeRange || Math.abs(dy) > seeRange || Math.abs(dz) > 500) {
                            if (!_seenCreatures.isEmpty()) {
                                _seenCreatures.remove(pl.getObjectId());
                            }
                            return;
                        }

                        final boolean isInRange = (dx * dx + dy * dy + dz * dz) <= seeRangeSq;
                        ...
                        // Player is legitimately within seeRange: now perform status and zone checks
                        if (ConfigNpcs.MOB_AGGRO_IN_PEACEZONE && _actor instanceof Monster monster && monster.getTemplate().getAggro() && pl.isInsideZone(ZoneId.PEACE)) {
                        ...
                        final int plId = pl.getObjectId();
                        if (!_actor.isRaidBoss() && _seenCreatures.contains(plId)) {
                            if (_actor instanceof Guard) {
                                _seenCreatures.remove(plId);
                            }
                        } else if (_actor.isRaidBoss() && Math.abs(dz) <= 500) {
