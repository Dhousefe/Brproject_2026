...
public final class RequestStartPledgeWar extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (attackerClan.getLevel() < 3 || attackerClan.getMembersCount() < ConfigClans.CLAN_MEMBERS_FOR_WAR)
		{
			player.sendPacket(SystemMessageId.CLAN_WAR_DECLARED_IF_CLAN_LVL3_OR_15_MEMBER);
			return;
		}
		
		if (!attackerClan.getAttackerList().contains(attackedClan.getClanId()) && (attackedClan.getLevel() < 3 || attackedClan.getMembersCount() < ConfigClans.CLAN_MEMBERS_FOR_WAR))
		{
