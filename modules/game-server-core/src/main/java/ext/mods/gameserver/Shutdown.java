...
public class Shutdown extends Thread
{
	...
	public void run()
	{
		if (this == SingletonHolder.INSTANCE)
		{
			...
			try
			{
				if (GameServer.getInstance().getSelectorThread() != null)
				{
			...
			while (_secondsShut > 0)
			{
				if (_shutdownMode == ABORT)
				{
					if (LoginServerThread.getInstance().getServerType() == ServerType.DOWN)
						LoginServerThread.getInstance().setServerType((ConfigServer.SERVER_GMONLY) ? ServerType.GM_ONLY : ServerType.AUTO);
	...
	public void playServerLoadedSound()
	{
		...
		{
			LOGGER.error("Failed to play shutdown sound", e);
		}
	}
