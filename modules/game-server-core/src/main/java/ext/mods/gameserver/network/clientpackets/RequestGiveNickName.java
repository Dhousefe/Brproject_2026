...
public class RequestGiveNickName extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (!StringUtil.isValidString(_title, ConfigServer.TITLE_TEMPLATE))
		{
			player.sendPacket(SystemMessageId.NOT_WORKING_PLEASE_TRY_AGAIN_LATER);
			return;
		}
		
		if (ConfigProject.AUTOFARM_CHANGE_PLAYER_TITLE && AutoFarmManager.getInstance().isPlayerActive(player.getObjectId()))
			return;
