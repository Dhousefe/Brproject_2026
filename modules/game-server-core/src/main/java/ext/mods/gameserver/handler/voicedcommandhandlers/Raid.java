...
public class Raid implements IVoicedCommandHandler
{
	...
	{
		bosses = IntStream.of(ConfigNpcs.RAID_BOSS_LIST).boxed().map(id -> NpcData.getInstance().getTemplate(id)).filter(o -> o != null).sorted(Comparator.comparing(NpcTemplate::getLevel)).toList();
	}
	...
	{
		if (!ConfigProject.SHOW_RAID_HTM)
		{
		...
		for (NpcTemplate npc : list)
		{
			...
			final String teleLink = ConfigProject.RAID_BOSS_TELEPORT_ENABLED
	...
	{
		if (!ConfigProject.RAID_BOSS_TELEPORT_ENABLED)
			return;
