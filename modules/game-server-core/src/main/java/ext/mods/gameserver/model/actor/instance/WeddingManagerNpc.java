...
public class WeddingManagerNpc extends Folk
{
	...
	private boolean weddingConditions(Player requester, Player partner)
	{
		...
		if (!ConfigNpcs.WEDDING_SAMESEX && partner.getAppearance().getSex() == requester.getAppearance().getSex())
		{
		...
		if (ConfigNpcs.WEDDING_FORMALWEAR && (!requester.isWearingFormalWear() || !partner.isWearingFormalWear()))
		{
			sendHtmlMessage(requester, "html/mods/wedding/error_noformal.htm");
			return false;
		}
		
		if (requester.getAdena() < ConfigNpcs.WEDDING_PRICE || partner.getAdena() < ConfigNpcs.WEDDING_PRICE)
		{
	...
	public static void justMarried(Player requester, Player partner)
	{
		...
		requester.reduceAdena(ConfigNpcs.WEDDING_PRICE, true);
		partner.reduceAdena(ConfigNpcs.WEDDING_PRICE, true);
	...
	private void sendHtmlMessage(Player player, String file)
	{
		...
		html.replace("%adenaCost%", StringUtil.formatNumber(ConfigNpcs.WEDDING_PRICE));
		html.replace("%needOrNot%", ConfigNpcs.WEDDING_FORMALWEAR ? player.getSysString(10_068) : player.getSysString(10_069));
