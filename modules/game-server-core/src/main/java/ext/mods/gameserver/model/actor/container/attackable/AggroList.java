...
public class AggroList extends ConcurrentHashMap<Creature, AggroInfo>
{
	...
	public Creature reconsiderTarget(int range)
	{
		...
		{
			final int aggroRange = _owner.getTemplate().getAggroRange();
			final int effectiveRange = (range > 0 && range < aggroRange) ? range : aggroRange;
			
			for (Creature creature : _owner.getKnownTypeInRadius(Creature.class, effectiveRange))
			{
	...
	public void randomizeAttack()
	{
		...
		AggroInfo randomInfo = null;
		for (AggroInfo a : values())
		{
			if (a != mostHatedInfo && a.getHate() > 0 && _owner.canAutoAttack(a.getAttacker(), ConfigPlayers.PARTY_RANGE, true))
			{
				randomInfo = a;
				break;
			}
