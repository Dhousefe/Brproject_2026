...
import ext.mods.config.ConfigBossJewels;
	...
	public void useAdminCommand(String command, Player player)
	{
		...
		if (st.countTokens() == 2)
		{
			try
			{
				...
				if (ConfigBossJewels.UPGRADE_BOSS_JEWELS_ENCHANT && ConfigBossJewels.UPGRADEABLE_BOSS_JEWELS.contains(item.getItemId())) {
				    int currentEnchant = item.getEnchantLevel();
				    for (Config.JewelUpgrade upgrade : ConfigBossJewels.UPGRADE_BOSS_JEWELS) {
