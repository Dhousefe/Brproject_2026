...
public class DMManager
{
	...
	private DMManager()
	{
		if (ConfigEvents.DM_EVENT_ENABLED)
		{
	...
	public void scheduleEventStart()
	{
		try
		{
			...
			for (String timeOfDay : ConfigEvents.DM_EVENT_INTERVAL)
			{
	...
	public void startReg()
	{
		...
		{
			World.announceToOnlinePlayers("Deathmatch: Joinable in " + ConfigEvents.DM_NPC_LOC_NAME + "!");
			
			if (ConfigEvents.EVENT_COMMANDS)
				World.announceToOnlinePlayers("Deathmatch: Command: .dmjoin / .dmleave / .dminfo");
			
			if (ConfigEvents.ALLOW_DM_DLG)
			{
			...
			_task.setStartTime(System.currentTimeMillis() + (60000L * ConfigEvents.DM_EVENT_PARTICIPATION_TIME));
			ThreadPool.execute(_task);
		}
	}
	...
	public void startEvent()
	{
		...
		{
			DMEvent.getInstance().sysMsgToAllParticipants("Teleporting in " + ConfigEvents.DM_EVENT_START_LEAVE_TELEPORT_DELAY + " second(s).");
			_task.setStartTime(System.currentTimeMillis() + 60000L * ConfigEvents.DM_EVENT_RUNNING_TIME);
			ThreadPool.execute(_task);
		}
	}
	...
	public void endEvent()
	{
		...
		DMEvent.getInstance().sysMsgToAllParticipants("Teleporting back town in " + ConfigEvents.DM_EVENT_START_LEAVE_TELEPORT_DELAY + " second(s).");
	...
	public String getTutorialAlertHtml(Player player)
	{
		...
		String message = DMEvent.getInstance().isParticipating()
			? ("Registration is open! Join in " + ConfigEvents.DM_NPC_LOC_NAME + ". Commands: .dmjoin / .dmleave / .dminfo")
