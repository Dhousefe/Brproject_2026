...
public enum ClassId
{
	...
	private final void createSubclasses()
	{
		...
		for (ClassId classId : VALUES)
		{
			...
			if (!ConfigProject.CLASS_OVERLORD && (classId == OVERLORD || classId == WARSMITH))
				continue;
			
			if (classId == this)
				continue;
			
			if (!ConfigProject.RACE_ELF && (_race == ClassRace.ELF && classId._race == ClassRace.DARK_ELF) || (_race == ClassRace.DARK_ELF && classId._race == ClassRace.ELF))
				continue;
			
			_subclasses.add(classId);
		}
		
		if (!ConfigProject.RESTRICTED_CLASSES)
		{
			switch (this)
			{
