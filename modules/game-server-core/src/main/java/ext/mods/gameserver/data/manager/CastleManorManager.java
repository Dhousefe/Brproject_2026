...
public class CastleManorManager implements IXmlReader
{
	...
	protected CastleManorManager()
	{
		...
		final int maintenanceMin = ConfigClans.MANOR_REFRESH_MIN + ConfigClans.MANOR_MAINTENANCE_MIN;
		
		if ((hour >= ConfigClans.MANOR_REFRESH_TIME && min >= maintenanceMin) || hour < ConfigClans.MANOR_APPROVE_TIME || (hour == ConfigClans.MANOR_APPROVE_TIME && min <= ConfigClans.MANOR_APPROVE_MIN))
			_mode = ManorStatus.MODIFIABLE;
		else if (hour == ConfigClans.MANOR_REFRESH_TIME && (min >= ConfigClans.MANOR_REFRESH_MIN && min < maintenanceMin))
			_mode = ManorStatus.MAINTENANCE;
		
		scheduleModeChange();
		
		ThreadPool.scheduleAtFixedRate(this::storeMe, ConfigClans.MANOR_SAVE_PERIOD_RATE, ConfigClans.MANOR_SAVE_PERIOD_RATE);
	...
	private final void scheduleModeChange()
	{
		...
		{
			case MODIFIABLE:
				_nextModeChange.set(Calendar.HOUR_OF_DAY, ConfigClans.MANOR_APPROVE_TIME);
				_nextModeChange.set(Calendar.MINUTE, ConfigClans.MANOR_APPROVE_MIN);
			...
			case MAINTENANCE:
				_nextModeChange.set(Calendar.HOUR_OF_DAY, ConfigClans.MANOR_REFRESH_TIME);
				_nextModeChange.set(Calendar.MINUTE, ConfigClans.MANOR_REFRESH_MIN + ConfigClans.MANOR_MAINTENANCE_MIN);
				break;
			
			case APPROVED:
				_nextModeChange.set(Calendar.HOUR_OF_DAY, ConfigClans.MANOR_REFRESH_TIME);
				_nextModeChange.set(Calendar.MINUTE, ConfigClans.MANOR_REFRESH_MIN);
				break;
		}
