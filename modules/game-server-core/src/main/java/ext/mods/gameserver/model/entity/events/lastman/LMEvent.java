...
public class LMEvent extends Events
{
	...
	{
		NpcTemplate tmpl = NpcData.getInstance().getTemplate(ConfigEvents.LM_EVENT_PARTICIPATION_NPC_ID);
		...
		try
		{
			...
			_npcSpawn.setLoc(ConfigEvents.LM_EVENT_PARTICIPATION_NPC_COORDINATES[0], ConfigEvents.LM_EVENT_PARTICIPATION_NPC_COORDINATES[1], ConfigEvents.LM_EVENT_PARTICIPATION_NPC_COORDINATES[2], ConfigEvents.LM_EVENT_PARTICIPATION_NPC_COORDINATES[3]);
	...
	public boolean startFight()
	{
		...
		if (getPlayerCounts() < ConfigEvents.LM_EVENT_MIN_PLAYERS)
		{
			setState(EventState.INACTIVE);
			
			for (LMPlayer player : _player.values())
				player.getPlayer().addItem(ConfigEvents.LM_EVENT_PARTICIPATION_FEE[0], ConfigEvents.LM_EVENT_PARTICIPATION_FEE[1], true);
		...
		closeDoors(ConfigEvents.LM_DOORS_IDS_TO_CLOSE);
	...
	public String calculateRewards()
	{
		...
		if (!ConfigEvents.LM_REWARD_PLAYERS_TIE && getPlayerCounts() > 1)
		...
		for (int i = 0; i < players.size(); i++)
		{
			...
			msg += " Died: " + String.valueOf(ConfigEvents.LM_EVENT_PLAYER_CREDITS - player.getCredits());
			msg += "\n";
			if (!ConfigEvents.LM_REWARD_PLAYERS_TIE)
				break;
		}
	...
	private void rewardPlayer(LMPlayer lmplayer)
	{
		...
		if (ConfigEvents.LM_EVENT_HERO)
			setHero(player, ConfigEvents.LV_EVENT_HERO_DAYS);
		
		SystemMessage systemMessage = null;
		
		for (IntIntHolder reward : ConfigEvents.LM_EVENT_REWARDS)
		{
	...
	public void stopFight()
	{
		...
		openDoors(ConfigEvents.LM_DOORS_IDS_TO_CLOSE);
		
		for (LMPlayer player : _player.values())
		{
			if (player != null)
				new LMEventTeleporter(player.getPlayer(), ConfigEvents.LM_EVENT_PARTICIPATION_NPC_COORDINATES, false, false);
		}
	...
	{
		return player.destroyItemByItemId(ConfigEvents.LM_EVENT_PARTICIPATION_FEE[0], ConfigEvents.LM_EVENT_PARTICIPATION_FEE[1], true);
	}
	
	public String getParticipationFee()
	{
		int itemId = ConfigEvents.LM_EVENT_PARTICIPATION_FEE[0];
		int itemNum = ConfigEvents.LM_EVENT_PARTICIPATION_FEE[1];
	...
	public void onLogout(Player player)
	{
		...
		{
			if (SafeDisconnectHooks.get().isSafeDisconnectActive(player) && ConfigSafeDisconnect.SAFEDISCONNECT_INTEGRATION_LM)
				return;
			
			if (removeParticipant(player))
				player.teleportTo(ConfigEvents.LM_EVENT_PARTICIPATION_NPC_COORDINATES[0] + Rnd.get(101) - 50, ConfigEvents.LM_EVENT_PARTICIPATION_NPC_COORDINATES[1] + Rnd.get(101) - 50, ConfigEvents.LM_EVENT_PARTICIPATION_NPC_COORDINATES[2], 0);
		}
	}
	...
	public synchronized void onBypass(String command, Player player)
	{
		...
		if (command.equals("lm_event_participation"))
		{
			...
			else if (ConfigEvents.DISABLE_ID_CLASSES_LM.contains(player.getClassId().getId()))
			{
			...
			else if (playerLevel < ConfigEvents.LM_EVENT_MIN_LVL || playerLevel > ConfigEvents.LM_EVENT_MAX_LVL)
			{
				htmContent = HTMLData.getInstance().getHtm(player, htmlPath + "Level.htm");
				if (htmContent != null)
				{
					npcHtmlMessage.setHtml(htmContent);
					npcHtmlMessage.replace("%min%", String.valueOf(ConfigEvents.LM_EVENT_MIN_LVL));
					npcHtmlMessage.replace("%max%", String.valueOf(ConfigEvents.LM_EVENT_MAX_LVL));
				}
			}
			else if (getPlayerCounts() == ConfigEvents.LM_EVENT_MAX_PLAYERS)
			{
				htmContent = HTMLData.getInstance().getHtm(player, htmlPath + "Full.htm");
				if (htmContent != null)
				{
					npcHtmlMessage.setHtml(htmContent);
					npcHtmlMessage.replace("%max%", String.valueOf(ConfigEvents.LM_EVENT_MAX_PLAYERS));
				}
			}
			else if ((ConfigEvents.LM_EVENT_MAX_PARTICIPANTS_PER_IP > 0) && !AntiFeedManager.getInstance().tryAddPlayer(AntiFeedManager.LM_ID, player, ConfigEvents.LM_EVENT_MAX_PARTICIPANTS_PER_IP))
			{
				htmContent = HTMLData.getInstance().getHtm(player, htmlPath + "IPRestriction.htm");
				if (htmContent != null)
				{
					npcHtmlMessage.setHtml(htmContent);
					npcHtmlMessage.replace("%max%", String.valueOf(AntiFeedManager.getInstance().getLimit(player, ConfigEvents.LM_EVENT_MAX_PARTICIPANTS_PER_IP)));
				}
			}
	...
	public boolean onScrollUse(int objectId)
	{
		...
		return isPlayerParticipant(objectId) && !ConfigEvents.LM_EVENT_SCROLL_ALLOWED;
	}
	...
	public boolean onPotionUse(int objectId)
	{
		...
		return isPlayerParticipant(objectId) && !ConfigEvents.LM_EVENT_POTIONS_ALLOWED;
	}
	...
	public boolean onItemSummon(int objectId)
	{
		...
		return isPlayerParticipant(objectId) && !ConfigEvents.LM_EVENT_SUMMON_BY_ITEM_ALLOWED;
	}
	...
	public void onTeleported(Player player)
	{
		...
		Map<Integer, Integer> buffs = player.isMageClass() ? ConfigEvents.LM_EVENT_MAGE_BUFFS : ConfigEvents.LM_EVENT_FIGHTER_BUFFS;
