...
public final class RequestCharacterCreate extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (ConfigProject.LIST_RESTRICTED_CHAR_NAMES.contains(_name.toLowerCase()))
		{
			sendPacket(CharCreateFail.REASON_INCORRECT_NAME);
			return;
		}
		
		if (!StringUtil.isValidString(_name, ConfigServer.CNAME_TEMPLATE))
		{
			sendPacket(CharCreateFail.REASON_INCORRECT_NAME);
			return;
		}
		...
		if (!tutorial.isEmpty())
		{
			QuestKillState.setCompleted(player, 1, false);
		}
		QuestKillState.setNotifyHtml(player, false);
		QuestKillState.setNotifyChat(player, false);
