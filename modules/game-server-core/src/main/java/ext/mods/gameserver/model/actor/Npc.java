...
public class Npc extends Creature
{
	...
	public void onSpawn()
	{
		...
		_currentSsCount = ConfigProject.NPC_SOULSHOT ? getTemplate().getAiParams().getInteger("SoulShot", 0) : 0;
		_currentSpsCount = ConfigProject.NPC_SPIRITSHOT ? getTemplate().getAiParams().getInteger("SpiritShot", 0) : 0;
	...
	public void showTeleportWindow(Player player, TeleportType type)
	{
		...
		for (int index = 0; index < teleports.size(); index++)
		{
			...
			if (!(ConfigNpcs.FREE_TELEPORT && player.getStatus().getLevel() <= ConfigNpcs.LVL_FREE_TELEPORT))
			{
	...
	{
		if (isPremium == 1)
			return (int) (getTemplate().getRewardExp() * ConfigRates.PREMIUM_RATE_XP);
		
		return (int) (getTemplate().getRewardExp() * ConfigRates.RATE_XP);
	}
	...
	{
		if (isPremium == 1)
			return (int) (getTemplate().getRewardSp() * ConfigRates.PREMIUM_RATE_SP);
		
		return (int) (getTemplate().getRewardSp() * ConfigRates.RATE_SP);
	}
	...
	public boolean canAutoAttack(Creature target, int range, boolean allowPeaceful)
	{
		...
		if (this instanceof Guard)
		{
			...
			if (target instanceof Monster targetMonster && ConfigNpcs.GUARD_ATTACK_AGGRO_MOB)
		...
		else
		{
			...
			if (!ConfigNpcs.MOB_AGGRO_IN_PEACEZONE && target.isInsideZone(ZoneId.PEACE))
				return false;
	...
	{
		return ConfigNpcs.DEFAULT_SEE_RANGE;
	}
