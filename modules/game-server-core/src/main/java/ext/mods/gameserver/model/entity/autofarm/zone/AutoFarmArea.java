...
public class AutoFarmArea extends ZoneType
{
	...
	private void moveToBufferAndReturn(Player player, Npc bufferNpc)
	{
		...
		player.setTarget(bufferNpc);
		final Location npcLoc = bufferNpc.getPosition();
		final boolean moveStarted = player.getMove().maybePlayerMoveToLocation(npcLoc, BUFFER_INTERACTION_RANGE_MAX, ConfigGeoengine.SISTEMA_PATHFINDING, false);
		deathReturnLog("{} moveToBufferAndReturn() Player.getMove().maybePlayerMoveToLocation(loc={} offset={} pathfinding={}) returned {}",
			DEATH_RETURN_TAG, npcLoc, BUFFER_INTERACTION_RANGE_MAX, ConfigGeoengine.SISTEMA_PATHFINDING, moveStarted);
		...
		taskHolder[0] = ThreadPool.scheduleAtFixedRate(() ->
		{
			...
			if (!inRange && !timeout && bufferNpc != null)
			{
				...
				if (now - lastMoveRetry[0] >= 3000)
				{
					lastMoveRetry[0] = now;
					player.setTarget(bufferNpc);
					player.getMove().maybePlayerMoveToLocation(bufferNpc.getPosition(), BUFFER_INTERACTION_RANGE_MAX, ConfigGeoengine.SISTEMA_PATHFINDING, false);
