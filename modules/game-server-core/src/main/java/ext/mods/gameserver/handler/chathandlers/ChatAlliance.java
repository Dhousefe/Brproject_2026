...
public class ChatAlliance implements IChatHandler
{
	...
	public void handleChat(SayType type, Player player, String target, String text)
	{
		...
		clan.broadcastToAllyMembers(new CreatureSay(player.getObjectId(), type, displayName, safeText));

		// Inject into site ClanChatRing for all clans in this alliance so web users see alliance chat
		try {
			for (Clan allyClan : ClanTable.getInstance().getClans())
			{
				if (allyClan != null && allyClan.getAllyId() == clan.getAllyId())
				{
					ClanChatRing.INSTANCE.append(allyClan.getClanId(), player.getObjectId(), player.getName(), safeText, role, "ALLY");
				}
			}
