...
public class Guard extends DefaultNpc
{
	...
	{
		if (ConfigNpcs.ENABLE_GUARD_CHAT)
		{
			_isChattyGuard = Rnd.get(100) < ConfigNpcs.GUARD_CHATTY_CHANCE;
			_lastAggroSay = 0;
			_lastNormalSay = 0;
	...
	{
		if (ConfigNpcs.ENABLE_GUARD_CHAT && _isChattyGuard && attacker instanceof Player player)
		{
			if (player.getKarma() > 0 && System.currentTimeMillis() > _lastAggroSay + ConfigNpcs.GUARD_SAY_AGGRO_PERIOD)
			{
	...
	public void onSeeCreature(Npc npc, Creature creature)
	{
		...
		{
			if (ConfigNpcs.ENABLE_GUARD_CHAT && _isChattyGuard && System.currentTimeMillis() > _lastAggroSay + ConfigNpcs.GUARD_SAY_AGGRO_PERIOD)
			{
				sayAggressiveMessage(npc, player);
				_lastAggroSay = System.currentTimeMillis();
			}
	...
	public String onTimer(String name, Npc npc, Player player)
	{
		...
		{
			if (ConfigNpcs.ENABLE_GUARD_CHAT && _isChattyGuard && !npc.isDead() && npc.getAI().getCurrentIntention().getType() != IntentionType.ATTACK)
			{
				if (System.currentTimeMillis() > _lastNormalSay + ConfigNpcs.GUARD_SAY_NORMAL_PERIOD)
				{
					for (Player nearbyPlayer : npc.getKnownTypeInRadius(Player.class, ConfigNpcs.GUARD_CHAT_RANGE))
					{
						if (nearbyPlayer != null && nearbyPlayer.getKarma() == 0 && nearbyPlayer.getAppearance().isVisible())
						{
							if (Rnd.get(100) < ConfigNpcs.GUARD_SAY_NORMAL_CHANCE)
							{
		...
		else if (name.equalsIgnoreCase("1671"))
		{
			if (npc.getStatus().getHpRatio() > 0.4 && !npc.isDead() && npc.getAI().getCurrentIntention().getType() != IntentionType.ATTACK)
			{
				...
				{
					if (moveAroundSocial > 0 && Rnd.get(100) < ConfigNpcs.NPC_ANIMATION)
						npc.getAI().addSocialDesire(3, (moveAroundSocial * 1000) / 30, 50);
					else if (moveAroundSocial1 > 0 && Rnd.get(100) < ConfigNpcs.NPC_ANIMATION)
