...
public final class ItemsOnGroundTaskManager implements Runnable
{
	...
	public final void add(ItemInstance item, Creature creature)
	{
		...
		if (ConfigServer.ITEMS_GC_CLEANUP_ENABLED && ConfigServer.ITEMS_GC_CLEANUP_TIME_MS > 0)
		{
			dropTime = ConfigServer.ITEMS_GC_CLEANUP_TIME_MS;
		}
		else
		{
			Integer special = ConfigServer.SPECIAL_ITEM_DESTROY_TIME.get(item.getItemId());
			if (special != null)
				dropTime = special;
			else if (item.isHerb())
				dropTime = ConfigServer.HERB_AUTO_DESTROY_TIME;
			else if (item.isEquipable())
				dropTime = ConfigServer.EQUIPABLE_ITEM_AUTO_DESTROY_TIME;
			else
			{
				final Castle castle = CastleManager.getInstance().getCastle(item);
				dropTime = (castle != null && castle.getTicket(item.getItemId()) != null) ? 0 : ConfigServer.ITEM_AUTO_DESTROY_TIME;
			}
			
			if (creature instanceof Playable)
				dropTime *= ConfigServer.PLAYER_DROPPED_ITEM_MULTIPLIER;
		}
