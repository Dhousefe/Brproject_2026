...
public class Pet extends Summon
{
	...
	{
		getStatus().addExpAndSp(Math.round(addToExp * ((getNpcId() == 12564) ? ConfigRates.SINEATER_XP_RATE : ConfigRates.PET_XP_RATE)), addToSp);
	}
	...
	{
		return (int) getStatus().calcStat(Stats.WEIGHT_LIMIT, 34500 * Formulas.CON_BONUS[getStatus().getCON()] * ConfigPlayers.WEIGHT_LIMIT, this, null);
	}
	...
	{
		return ConfigPlayers.INVENTORY_MAXIMUM_PET;
	}
