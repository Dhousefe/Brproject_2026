...
public abstract class PlayableAI<T extends Playable> extends CreatureAI<T>
{
	...
	protected final AtomicReference<Intention> _nextIntentionRef = new AtomicReference<>(null);
	...
	/**
	 * Atomically schedules the next intention without lock tearing.
	 * Updates the atomic reference and synchronizes the legacy fallback.
	 * @param intention The new {@link Intention} to queue.
	 */
	protected void scheduleNextIntention(Intention intention)
	{
		_nextIntentionRef.set(intention);
		_nextIntention.updateUsing(intention);
	}

	/**
	 * Atomically retrieves and resets the pending next intention.
	 * Returns null if no valid next intention was queued.
	 */
	protected Intention consumeNextIntention()
	{
		final Intention atomicNext = _nextIntentionRef.getAndSet(null);
		if (atomicNext != null && !atomicNext.isBlank())
		{
		...
		{
			final Intention fallback = new Intention();
			fallback.updateUsing(_nextIntention);
			_nextIntention.updateAsIdle();
			return fallback;
		}
		return null;
	...
	{
		final Intention next = _nextIntentionRef.get();
		return next != null ? next : super.getNextIntention();
	}

	@Override
	protected void setNextIntention(Intention intention)
	{
		if (intention == null || intention.isBlank())
		{
			_nextIntentionRef.set(null);
			super.setNextIntention(intention);
		}
		else
		{
			final Intention copy = new Intention();
			copy.updateUsing(intention);
			scheduleNextIntention(copy);
		}
	}
	...
	public synchronized void tryToAttack(Creature target, boolean isCtrlPressed, boolean isShiftPressed)
	{
		...
		{
			final Intention next = new Intention();
			next.updateAsAttack(target, isCtrlPressed, isShiftPressed, true);
			scheduleNextIntention(next);
			clientActionFailed();
			return;
		}
	...
	public synchronized void tryToCast(Creature target, L2Skill skill, boolean isCtrlPressed, boolean isShiftPressed, int itemObjectId)
	{
		...
		{
			final Intention next = new Intention();
			next.updateAsCast(_actor, target, skill, isCtrlPressed, isShiftPressed, itemObjectId, true);
			scheduleNextIntention(next);
			clientActionFailed();
			return;
		}
	...
	public synchronized void tryToFollow(Creature target, boolean isShiftPressed)
	{
		...
		{
			final Intention next = new Intention();
			next.updateAsFollow(target, isShiftPressed);
			scheduleNextIntention(next);
			clientActionFailed();
			return;
		}
	...
	public synchronized void tryToInteract(WorldObject target, boolean isCtrlPressed, boolean isShiftPressed)
	{
		...
		{
			final Intention next = new Intention();
			next.updateAsInteract(target, isCtrlPressed, isShiftPressed);
			scheduleNextIntention(next);
			clientActionFailed();
			return;
		}
	...
	public synchronized void tryToPickUp(int itemObjectId, boolean isShiftPressed)
	{
		...
		{
			final Intention next = new Intention();
			next.updateAsPickUp(itemObjectId, isShiftPressed);
			scheduleNextIntention(next);
			clientActionFailed();
			return;
		}
	...
	public synchronized void tryToUseItem(int itemObjectId)
	{
		...
		{
			final Intention next = new Intention();
			next.updateAsUseItem(itemObjectId);
			scheduleNextIntention(next);
			return;
		}
