...
public class LMManager
{
	...
	private LMManager()
	{
		if (ConfigEvents.LM_EVENT_ENABLED)
		{
	...
	public void scheduleEventStart()
	{
		try
		{
			...
			for (String timeOfDay : ConfigEvents.LM_EVENT_INTERVAL)
			{
	...
	public void startReg()
	{
		...
		{
			World.announceToOnlinePlayers("Last Man: Joinable in " + ConfigEvents.LM_NPC_LOC_NAME + "!");
			
			if (ConfigEvents.EVENT_COMMANDS)
				World.announceToOnlinePlayers("Last Man: Command: .lmjoin / .lmleave / .lminfo");
			
			if (ConfigEvents.ALLOW_LM_DLG)
			{
			...
			_task.setStartTime(System.currentTimeMillis() + (60000L * ConfigEvents.LM_EVENT_PARTICIPATION_TIME));
			ThreadPool.execute(_task);
		}
	}
	...
	public void startEvent()
	{
		...
		{
			LMEvent.getInstance().sysMsgToAllParticipants("Teleporting participants in " + ConfigEvents.LM_EVENT_START_LEAVE_TELEPORT_DELAY + " second(s).");
			_task.setStartTime(System.currentTimeMillis() + 60000L * ConfigEvents.LM_EVENT_RUNNING_TIME);
			ThreadPool.execute(_task);
		}
	}
	...
	public void endEvent()
	{
		...
		LMEvent.getInstance().sysMsgToAllParticipants("Teleporting back to town in " + ConfigEvents.LM_EVENT_START_LEAVE_TELEPORT_DELAY + " second(s).");
	...
	public String getTutorialAlertHtml(Player player)
	{
		...
		String message = LMEvent.getInstance().isParticipating()
			? ("Registration is open! Join in " + ConfigEvents.LM_NPC_LOC_NAME + ". Commands: .lmjoin / .lmleave / .lminfo")
