...
public class Teleport implements IBypassHandler
{
	...
	public void teleport(Player player, Npc npc, int index)
	{
		...
		if (ConfigNpcs.FREE_TELEPORT && player.getStatus().getLevel() <= ConfigNpcs.LVL_FREE_TELEPORT || teleport.getPriceCount() == 0 || player.destroyItemByItemId(teleport.getPriceId(), teleport.getCalculatedPriceCount(player), true))
