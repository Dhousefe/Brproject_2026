...
abstract class DocumentBase
{
	...
	protected void attachEffect(Node n, Object template, Condition attachCond)
	{
		...
		if (ConfigProject.ENABLE_MODIFY_SKILL_DURATION)
		{
			if (ConfigProject.SKILL_DURATION_LIST.containsKey(((L2Skill) template).getId()))
			{
				if (((L2Skill) template).getLevel() < 100)
				{
					time = ConfigProject.SKILL_DURATION_LIST.get(((L2Skill) template).getId());
				}
