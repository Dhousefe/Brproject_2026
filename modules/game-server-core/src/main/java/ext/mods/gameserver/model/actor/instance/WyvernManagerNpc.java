...
public class WyvernManagerNpc extends CastleChamberlain
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		if (command.startsWith("RideWyvern"))
		{
			...
			if (player.getMountLevel() < ConfigNpcs.WYVERN_REQUIRED_LEVEL)
			{
				sendHtm(player, "6");
				return;
			}
			
			if (!player.destroyItemByItemId(1460, ConfigNpcs.WYVERN_REQUIRED_CRYSTALS, true))
			{
				sendHtm(player, "5");
				return;
			}
	...
	private void sendHtm(Player player, String val)
	{
		...
		html.replace("%wyvern_level%", ConfigNpcs.WYVERN_REQUIRED_LEVEL);
		html.replace("%needed_crystals%", ConfigNpcs.WYVERN_REQUIRED_CRYSTALS);
