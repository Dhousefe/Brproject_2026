...
public final class GameTimeTaskManager implements Runnable
{
	...
	public final void run()
	{
		...
		if (nightChanged)
		{
			final var transitionPacket = _isNight ? SunSet.STATIC_PACKET : SunRise.STATIC_PACKET;
			for (Player player : players)
			{
				if (player.isOnline())
					player.sendPacket(transitionPacket);
			}
		}
	...
	{
		int newTime = (hours * 60 + minutes) % MINUTES_PER_DAY;
		if (newTime < 0)
			newTime += MINUTES_PER_DAY;
		
		_time = newTime;
		
		final boolean oldNight = _isNight;
		_isNight = isNight();
		
		final ClientSetTime cst = new ClientSetTime();
		final boolean nightChanged = (oldNight != _isNight);
		final var transition = nightChanged ? (_isNight ? SunSet.STATIC_PACKET : SunRise.STATIC_PACKET) : null;
		
		for (Player player : World.getInstance().getPlayers())
		{
			if (player.isOnline())
			{
				player.sendPacket(cst);
				if (transition != null)
					player.sendPacket(transition);
			}
		}
	}
	
	public void forceSunRise()
	{
		setGameTime(6, 0);
		for (Player player : World.getInstance().getPlayers())
		{
	...
	public void forceSunSet()
	{
		...
		{
			if (player.isOnline())
				player.sendPacket(SunSet.STATIC_PACKET);
		}
	}
