...
public class RaidPrivateHealer extends RaidPrivateStandard
{
	...
	{
		if (!ConfigNpcs.RAID_DISABLE_CURSE && target.getStatus().getLevel() > (called.getStatus().getLevel() + 8))
	...
	public void onUseSkillFinished(Npc npc, Creature creature, L2Skill skill, boolean success)
	{
		...
		if (skill.getId() == healMagic_a.getId())
		{
			if (npc._c_ai0 != null && npc._c_ai0 != npc && npc.distance2D(npc._c_ai0) < 200)
				npc.getAI().addFleeDesire(npc._c_ai0, ConfigNpcs.MAX_DRIFT_RANGE, 100000000);
		}
	}
}