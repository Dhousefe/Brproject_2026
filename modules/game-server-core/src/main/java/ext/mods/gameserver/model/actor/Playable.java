...
public abstract class Playable extends Creature
{
	...
	public boolean doDie(Creature killer)
	{
		...
		else if (isNoblesseBlessed() || (LMEvent.getInstance().isStarted() && ConfigEvents.LM_EVENT_EFFECTS_REMOVAL == 2 || TvTEvent.getInstance().isStarted() && ConfigEvents.TVT_EVENT_EFFECTS_REMOVAL == 2 || DMEvent.getInstance().isStarted() && ConfigEvents.DM_EVENT_EFFECTS_REMOVAL == 2 || CTFEvent.getInstance().isStarted() && ConfigEvents.CTF_EVENT_EFFECTS_REMOVAL == 2))
		{
			stopNoblesseBlessing(null);
	...
	public void doRevive()
	{
		...
		if (isPhoenixBlessed())
		{
			...
			getStatus().setHp(getStatus().getMaxHp() * ConfigPlayers.RESPAWN_RESTORE_HP);
	...
	public boolean isAttackableWithoutForceBy(Playable attacker)
	{
		...
		if (this instanceof Player)
		{
			...
			if (TournamentState.isIn(player) && TournamentState.isIn(attackerPlayer))
			{
				return true;
			}
		}
	...
	{
		if (ConfigNpcs.RAID_DISABLE_CURSE || !(npc instanceof Attackable attackable))
			return false;
	...
	{
		if (ConfigNpcs.RAID_DISABLE_CURSE)
			return false;
