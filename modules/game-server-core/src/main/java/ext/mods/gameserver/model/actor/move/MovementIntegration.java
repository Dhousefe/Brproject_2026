...
public class MovementIntegration
{
    ...
    public static void initializeMovementSystem(Creature creature) { 
        if (!ConfigGeoengine.SISTEMA_PATHFINDING) return; 
        if (ConfigGeoengine.USE_OPTIMIZED_MOVEMENT && creature instanceof Player) creature.setMove(); 
    }
    ...
    public static boolean canSeeTarget(WorldObject object, WorldObject target) { 
        if (!ConfigGeoengine.SISTEMA_PATHFINDING) return true; 
        ...
        double oheight = (object instanceof Creature c) ? c.getCollisionHeight() * 2 * ConfigGeoengine.PART_OF_CHARACTER_HEIGHT / 100 : 0; 
        double theight = (target instanceof Creature c) ? c.getCollisionHeight() * 2 * ConfigGeoengine.PART_OF_CHARACTER_HEIGHT / 100 : 0; 
