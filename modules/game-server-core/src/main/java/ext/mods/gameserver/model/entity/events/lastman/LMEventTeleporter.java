...
public class LMEventTeleporter implements Runnable
{
	...
	{
		_player = player;
		_coordinates = ConfigEvents.LM_EVENT_PLAYER_COORDINATES.get(Rnd.get(ConfigEvents.LM_EVENT_PLAYER_COORDINATES.size()));
		_adminRemove = adminRemove;
	...
	{
		ThreadPool.schedule(this, fastSchedule ? 0 : (LMEvent.getInstance().isStarted() ? ConfigEvents.LM_EVENT_RESPAWN_TELEPORT_DELAY : ConfigEvents.LM_EVENT_START_LEAVE_TELEPORT_DELAY) * 1000);
	}
	...
	public void run()
	{
		...
		if (ConfigEvents.LM_EVENT_EFFECTS_REMOVAL == 0 || (ConfigEvents.LM_EVENT_EFFECTS_REMOVAL == 1 && (_player.getTeam() == TeamType.NONE || (_player.isInDuel() && _player.getDuelState() != DuelState.INTERRUPTED))))
