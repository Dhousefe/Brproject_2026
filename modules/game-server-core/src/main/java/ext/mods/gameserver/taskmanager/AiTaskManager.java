...
public final class AiTaskManager implements Runnable
{
	...
	private static final int RETURN_HOME_RAIDBOSS_RADIUS = ConfigProject.RETURN_HOME_RAIDBOSS_RADIUS;
	private static final int RETURN_HOME_MONSTER_RADIUS = ConfigProject.RETURN_HOME_MONSTER_RADIUS;
	...
	{
		if (ConfigGeoengine.ENABLE_NPC_MOVEMENT_OPTIMIZATION && !(npc instanceof GrandBoss || npc instanceof RaidBoss))
		{
		...
		if (npc instanceof GrandBoss)
			...
			monsterReturn(raidBoss, ConfigProject.RETURN_HOME_RAIDBOSS, RETURN_HOME_RAIDBOSS_RADIUS, EXCLUDED_RAIDBOSS_IDS);
		else if (npc instanceof Monster monster)
			monsterReturn(monster, ConfigProject.RETURN_HOME_MONSTER, RETURN_HOME_MONSTER_RADIUS, EXCLUDED_MONSTER_IDS);
	}
	...
	{
		if (ConfigGeoengine.NPC_MOVEMENT_PLAYER_RANGE <= 0)
			return true;

		return npc.hasKnownTypeInRadius(ext.mods.gameserver.model.actor.Player.class, ConfigGeoengine.NPC_MOVEMENT_PLAYER_RANGE, player -> player.getAppearance().isVisible());
	}
	...
	protected final void animationTask()
	{
		for (Npc npc : _npcs)
		{
			...
			if (folk.getStatus().getHpRatio() <= 0.4 || folk.isDead())
				continue;

			if (folk.getAI().getCurrentIntention().getType() == IntentionType.ATTACK)
				continue;

			if (Rnd.get(100) >= ConfigNpcs.NPC_ANIMATION)
				continue;
