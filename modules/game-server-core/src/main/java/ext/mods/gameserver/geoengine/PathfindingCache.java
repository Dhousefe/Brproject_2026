...
public class PathfindingCache
{
	...
	{
		if (!ConfigGeoengine.ENABLE_PATHFINDING_CACHE)
			return null;
		...
		if (System.currentTimeMillis() - cached.timestamp > ConfigGeoengine.PATHFINDING_CACHE_EXPIRATION)
		{
			_cache.remove(key);
			_misses.incrementAndGet();
			return null;
	...
	public static void cachePath(int ox, int oy, int oz, int tx, int ty, int tz, List<Location> path)
	{
		...
		if (_cache.size() >= ConfigGeoengine.PATHFINDING_CACHE_SIZE)
		{
			clearOldEntries();
		}
	...
	private static void clearOldEntries()
	{
		...
		final int toRemove = ConfigGeoengine.PATHFINDING_CACHE_SIZE / 4;
		int removed = 0;
		...
		for (var entry : _cache.entrySet())
		{
			...
			if (now - entry.getValue().timestamp > ConfigGeoengine.PATHFINDING_CACHE_EXPIRATION / 2)
			{
