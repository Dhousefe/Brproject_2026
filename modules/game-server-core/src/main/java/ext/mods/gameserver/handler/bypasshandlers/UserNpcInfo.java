...
public class UserNpcInfo implements IBypassHandler {
	...
	public static void showNpcStatsInfos(Player player, Npc npc, NpcHtmlMessage html)
	{
		...
		html.replace("%hp%", npc.isChampion() ? (int) npc.getStatus().getHp() * ConfigNpcs.CHAMPION_HP : (int) npc.getStatus().getHp());
		html.replace("%hpmax%", npc.isChampion() ? npc.getStatus().getMaxHp() * ConfigNpcs.CHAMPION_HP : npc.getStatus().getMaxHp());
		html.replace("%mp%", (int) npc.getStatus().getMp());
		html.replace("%mpmax%", npc.getStatus().getMaxMp());
		html.replace("%patk%", npc.isChampion() ? npc.getStatus().getPAtk(null) * ConfigNpcs.CHAMPION_ATK : npc.getStatus().getPAtk(null));
		html.replace("%matk%", npc.isChampion() ? npc.getStatus().getMAtk(null, null) * ConfigNpcs.CHAMPION_MATK : npc.getStatus().getMAtk(null, null));
		...
		html.replace("%aspd%", npc.isChampion() ? npc.getStatus().getPAtkSpd() * ConfigNpcs.CHAMPION_SPD_ATK : npc.getStatus().getPAtkSpd());
		html.replace("%cspd%", npc.isChampion() ? npc.getStatus().getMAtkSpd() * ConfigNpcs.CHAMPION_SPD_MATK : npc.getStatus().getMAtkSpd());
	...
	private static void showNpcInfoDrop(Player player, Npc npc, NpcHtmlMessage html, int page, int subPage, boolean isDrop)
	{
		...
		for (DropCategory category : list)
		{
			...
			if (ConfigProject.ALTERNATE_DROP_LIST)
			{
