...
import ext.mods.config.ConfigSafeDisconnect;
...
public class CTFEvent extends Events
{
	...
	public void init()
	{
		...
		_teams[0] = new CTFEventTeam(ConfigEvents.CTF_EVENT_TEAM_1_NAME, ConfigEvents.CTF_EVENT_TEAM_1_COORDINATES);
		_teams[1] = new CTFEventTeam(ConfigEvents.CTF_EVENT_TEAM_2_NAME, ConfigEvents.CTF_EVENT_TEAM_2_COORDINATES);
	}
	
	public boolean startParticipation()
	{
		final NpcTemplate tmpl = NpcData.getInstance().getTemplate(ConfigEvents.CTF_EVENT_PARTICIPATION_NPC_ID);
		...
		try
		{
			...
			_npcSpawn.setLoc(ConfigEvents.CTF_EVENT_PARTICIPATION_NPC_COORDINATES[0], ConfigEvents.CTF_EVENT_PARTICIPATION_NPC_COORDINATES[1], ConfigEvents.CTF_EVENT_PARTICIPATION_NPC_COORDINATES[2], ConfigEvents.CTF_EVENT_PARTICIPATION_NPC_COORDINATES[3]);
	...
	public boolean startFight()
	{
		...
		if ((_teams[0].getParticipatedPlayerCount() < ConfigEvents.CTF_EVENT_MIN_PLAYERS_IN_TEAMS) || (_teams[1].getParticipatedPlayerCount() < ConfigEvents.CTF_EVENT_MIN_PLAYERS_IN_TEAMS))
		{
			setState(EventState.INACTIVE);
			
			for (Player players : allParticipants.values())
				players.addItem(ConfigEvents.CTF_EVENT_PARTICIPATION_FEE[0], ConfigEvents.CTF_EVENT_PARTICIPATION_FEE[1], true);
		...
		closeDoors(ConfigEvents.CTF_DOORS_IDS_TO_CLOSE);
	...
	public String calculateRewards()
	{
		if (_teams[0].getPoints() == _teams[1].getPoints())
		{
			...
			if (ConfigEvents.CTF_REWARD_TEAM_TIE)
			{
				rewardTeam(_teams[0]);
				rewardTeam(_teams[1]);
	...
	private void rewardTeam(CTFEventTeam team)
	{
		for (Player player : team.getParticipatedPlayers().values())
		{
			...
			for (IntIntHolder reward : ConfigEvents.CTF_EVENT_REWARDS)
			{
	...
	public void stopFight()
	{
		...
		openDoors(ConfigEvents.CTF_DOORS_IDS_TO_CLOSE);
		...
		for (CTFEventTeam team : _teams)
		{
			...
			{
				if (player != null)
					new CTFEventTeleporter(player, ConfigEvents.CTF_EVENT_PARTICIPATION_NPC_COORDINATES, false, false);
			}
		}
	...
	{
		return (ConfigEvents.CTF_EVENT_PARTICIPATION_FEE[0] != 0) && (ConfigEvents.CTF_EVENT_PARTICIPATION_FEE[1] != 0);
	}
	
	public boolean hasParticipationFee(Player player)
	{
		return player.getInventory().getItemCount(ConfigEvents.CTF_EVENT_PARTICIPATION_FEE[0], -1) >= ConfigEvents.CTF_EVENT_PARTICIPATION_FEE[1];
	}
	
	public boolean payParticipationFee(Player player)
	{
		return player.destroyItemByItemId(ConfigEvents.CTF_EVENT_PARTICIPATION_FEE[0], ConfigEvents.CTF_EVENT_PARTICIPATION_FEE[1], true);
	}
	
	public String getParticipationFee()
	{
		int itemId = ConfigEvents.CTF_EVENT_PARTICIPATION_FEE[0];
		int itemNum = ConfigEvents.CTF_EVENT_PARTICIPATION_FEE[1];
	...
	{
		NpcTemplate tmpl = NpcData.getInstance().getTemplate(ConfigEvents.CTF_EVENT_TEAM_1_HEADQUARTERS_ID);
		...
		try
		{
			...
			_flag1Spawn.setLoc(ConfigEvents.CTF_EVENT_TEAM_1_FLAG_COORDINATES[0], ConfigEvents.CTF_EVENT_TEAM_1_FLAG_COORDINATES[1], ConfigEvents.CTF_EVENT_TEAM_1_FLAG_COORDINATES[2], ConfigEvents.CTF_EVENT_TEAM_1_FLAG_COORDINATES[3]);
			_flag1Spawn.setRespawnDelay(60000);
			
			SpawnManager.getInstance().addSpawn(_flag1Spawn);
			
			_lastFlag1Spawn = _flag1Spawn.doSpawn(false);
			_lastFlag1Spawn.setTitle(ConfigEvents.CTF_EVENT_TEAM_1_NAME);
		}
		catch (Exception e)
		{
	...
	{
		NpcTemplate tmpl = NpcData.getInstance().getTemplate(ConfigEvents.CTF_EVENT_TEAM_2_HEADQUARTERS_ID);
		...
		try
		{
			...
			_flag2Spawn.setLoc(ConfigEvents.CTF_EVENT_TEAM_2_FLAG_COORDINATES[0], ConfigEvents.CTF_EVENT_TEAM_2_FLAG_COORDINATES[1], ConfigEvents.CTF_EVENT_TEAM_2_FLAG_COORDINATES[2], ConfigEvents.CTF_EVENT_TEAM_2_FLAG_COORDINATES[3]);
			_flag2Spawn.setRespawnDelay(60000);
			
			SpawnManager.getInstance().addSpawn(_flag2Spawn);
			
			_lastFlag2Spawn = _flag2Spawn.doSpawn(false);
			_lastFlag2Spawn.setTitle(ConfigEvents.CTF_EVENT_TEAM_2_NAME);
		}
		catch (Exception e)
		{
	...
	public void onLogout(Player player)
	{
		...
		{
			if (SafeDisconnectHooks.get().isSafeDisconnectActive(player) && ConfigSafeDisconnect.SAFEDISCONNECT_INTEGRATION_CTF)
			{
			...
			if (removeParticipant(player.getObjectId()))
				player.teleportTo((ConfigEvents.CTF_EVENT_PARTICIPATION_NPC_COORDINATES[0] + Rnd.get(101)) - 50, (ConfigEvents.CTF_EVENT_PARTICIPATION_NPC_COORDINATES[1] + Rnd.get(101)) - 50, ConfigEvents.CTF_EVENT_PARTICIPATION_NPC_COORDINATES[2], 0);
		}
	}
	...
	public synchronized void onBypass(String command, Player player)
	{
		...
		if (command.equals("ctf_event_participation"))
		{
			...
			else if ((playerLevel < ConfigEvents.CTF_EVENT_MIN_LVL) || (playerLevel > ConfigEvents.CTF_EVENT_MAX_LVL))
			{
				htmContent = HTMLData.getInstance().getHtm(player, htmlPath + "Level.htm");
				if (htmContent != null)
				{
					npcHtmlMessage.setHtml(htmContent);
					npcHtmlMessage.replace("%min%", String.valueOf(ConfigEvents.CTF_EVENT_MIN_LVL));
					npcHtmlMessage.replace("%max%", String.valueOf(ConfigEvents.CTF_EVENT_MAX_LVL));
				}
			}
			else if ((_teams[0].getParticipatedPlayerCount() == ConfigEvents.CTF_EVENT_MAX_PLAYERS_IN_TEAMS) && (_teams[1].getParticipatedPlayerCount() == ConfigEvents.CTF_EVENT_MAX_PLAYERS_IN_TEAMS))
			{
				htmContent = HTMLData.getInstance().getHtm(player, htmlPath + "TeamsFull.htm");
				if (htmContent != null)
				{
					npcHtmlMessage.setHtml(htmContent);
					npcHtmlMessage.replace("%max%", String.valueOf(ConfigEvents.CTF_EVENT_MAX_PLAYERS_IN_TEAMS));
				}
			}
			else if ((ConfigEvents.CTF_EVENT_MAX_PARTICIPANTS_PER_IP > 0) && !AntiFeedManager.getInstance().tryAddPlayer(AntiFeedManager.CTF_ID, player, ConfigEvents.CTF_EVENT_MAX_PARTICIPANTS_PER_IP))
			{
				htmContent = HTMLData.getInstance().getHtm(player, htmlPath + "IPRestriction.htm");
				if (htmContent != null)
				{
					npcHtmlMessage.setHtml(htmContent);
					npcHtmlMessage.replace("%max%", String.valueOf(AntiFeedManager.getInstance().getLimit(player, ConfigEvents.CTF_EVENT_MAX_PARTICIPANTS_PER_IP)));
				}
			}
	...
	public boolean onAction(Player player, int objectId)
	{
		...
		if ((playerTeamId != -1) && (targetedPlayerTeamId != -1) && (playerTeamId == targetedPlayerTeamId) && (player.getObjectId() != objectId) && !ConfigEvents.CTF_EVENT_TARGET_TEAM_MEMBERS_ALLOWED)
		{
			player.sendPacket(ActionFailed.STATIC_PACKET);
			return false;
		}
	...
	public boolean onScrollUse(int objectId)
	{
		...
		return isPlayerParticipant(objectId) && !ConfigEvents.CTF_EVENT_SCROLL_ALLOWED;
	}
	...
	public boolean onPotionUse(int objectId)
	{
		...
		return isPlayerParticipant(objectId) && !ConfigEvents.CTF_EVENT_POTIONS_ALLOWED;
	}
	...
	public boolean onItemSummon(int objectId)
	{
		...
		return isPlayerParticipant(objectId) && !ConfigEvents.CTF_EVENT_SUMMON_BY_ITEM_ALLOWED;
	}
	...
	public void onTeleported(Player player)
	{
		...
		Map<Integer, Integer> buffs = player.isMageClass() ? ConfigEvents.CTF_EVENT_MAGE_BUFFS : ConfigEvents.CTF_EVENT_FIGHTER_BUFFS;
	...
	{
		return (_teams[0].containsPlayer(player.getObjectId()) ? ConfigEvents.CTF_EVENT_TEAM_2_FLAG : ConfigEvents.CTF_EVENT_TEAM_1_FLAG);
	}
