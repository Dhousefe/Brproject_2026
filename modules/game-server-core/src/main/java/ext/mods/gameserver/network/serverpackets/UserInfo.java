...
import ext.mods.gameserver.model.actor.player.PlayerGodState;
...
public class UserInfo extends L2GameServerPacket
{
	...
	protected final void writeImpl()
	{
		...
		IDressMeSkin armorSkin = DressMeState.armor(_player);
		IDressMeSkin weaponSkin = DressMeState.weapon(_player);
		...
		if (DressMeState.isActive(_player) && weaponSkin != null)
		{
		...
		writeC((ConfigPlayers.PLAYER_SPAWN_PROTECTION > 0 && _player.isSpawnProtected()) ? TeamType.BLUE.getId() : _player.getTeam().getId());
		writeD(_player.getClanCrestLargeId());
		writeC((_player.isNoble()) ? 1 : 0);
		writeC((_player.isHero() || PlayerGodState.hasHeroAura(_player) || (_player.isGM() && ConfigPlayers.GM_HERO_AURA)) ? 1 : 0);
