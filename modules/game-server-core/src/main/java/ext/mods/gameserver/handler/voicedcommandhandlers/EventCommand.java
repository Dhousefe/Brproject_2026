...
public class EventCommand implements IVoicedCommandHandler
{
	...
	private void showCTFStatusPage(Player player)
	{
		...
		html.replace("%team1name%", ConfigEvents.CTF_EVENT_TEAM_1_NAME);
		...
		html.replace("%team2name%", ConfigEvents.CTF_EVENT_TEAM_2_NAME);
	...
	{
		String[] firstPositions = DMEvent.getInstance().getFirstPosition(ConfigEvents.DM_REWARD_FIRST_PLAYERS);
		NpcHtmlMessage html = new NpcHtmlMessage(player.getObjectId());
	...
	private void showTvTStatusPage(Player player)
	{
		...
		html.replace("%team1name%", ConfigEvents.TVT_EVENT_TEAM_1_NAME);
		...
		html.replace("%team2name%", ConfigEvents.TVT_EVENT_TEAM_2_NAME);
