...
public class RequestGameStart extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if ((ConfigProject.DUALBOX_CHECK_MAX_PLAYERS_PER_IP > 0) && !AntiFeedManager.getInstance().tryAddClient(AntiFeedManager.GAME_ID, client, ConfigProject.DUALBOX_CHECK_MAX_PLAYERS_PER_IP))
		{
			final NpcHtmlMessage msg = new NpcHtmlMessage(0);
			msg.setFile(Locale.forLanguageTag("en-US"), "html/mods/IPRestriction.htm");
			msg.replace("%max%", String.valueOf(AntiFeedManager.getInstance().getLimit(client, ConfigProject.DUALBOX_CHECK_MAX_PLAYERS_PER_IP)));
