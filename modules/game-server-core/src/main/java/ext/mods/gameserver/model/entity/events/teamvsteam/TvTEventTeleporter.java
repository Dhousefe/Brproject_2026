...
public class TvTEventTeleporter implements Runnable
{
	...
	public TvTEventTeleporter(Player player, int[] coordinates, boolean fastSchedule, boolean adminRemove)
	{
		...
		ThreadPool.schedule(this, fastSchedule ? 0 : (TvTEvent.getInstance().isStarted() ? ConfigEvents.TVT_EVENT_RESPAWN_TELEPORT_DELAY : ConfigEvents.TVT_EVENT_START_LEAVE_TELEPORT_DELAY) * 1000);
	}
	...
	public void run()
	{
		...
		if ((ConfigEvents.TVT_EVENT_EFFECTS_REMOVAL == 0) || ((ConfigEvents.TVT_EVENT_EFFECTS_REMOVAL == 1) && ((_player.getTeam() == TeamType.NONE) || (_player.isInDuel() && (_player.getDuelState() != DuelState.INTERRUPTED)))))
