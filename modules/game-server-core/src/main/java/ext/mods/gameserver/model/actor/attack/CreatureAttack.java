...
public class CreatureAttack<T extends Creature> {
    ...
    private void onHitTimer() {
        ...
        long finalDelay = Math.max(100, (long) this._afterAttackDelay);
        ...
        switch (this._weaponType) {
            ...
            case DUALFIST: {
                ...
                this._attackTask = ThreadPool.schedule(() -> {
                    if (isAttackingNow()) {
                        doHit(_hitHolders[1]);
                        ThreadPool.schedule(() -> onFinishedAttack(mainTarget), finalDelay);
                    } else {
    ...
    public void doAttack(Creature target) {
        ...
        HitHolder[] hits = switch (_actor.getAttackType()) {
            ...
            case WeaponType.DUAL, WeaponType.DUALFIST -> this.doAttackHitByDual(target, weaponItem, timeAtk / 2, isSoulshot);
            case WeaponType.POLE -> this.doAttackHitByPole(target, weaponItem, timeAtk / 2, isSoulshot);
            default -> this.doAttackHitSimple(target, weaponItem, timeAtk / 2, isSoulshot);
        };
    ...
    private HitHolder[] doAttackHitByBow(Creature target, Weapon weapon, int sAtk, boolean isSoulshot) {
        final boolean infinityArrows = ConfigProject.INFINITY_ARROWS && (!ConfigProject.INFINITY_ARROWS_PREMIUM_ONLY
        ...
        int safeAtkTime = Math.max(100, sAtk);
        int recoveryReuse = Math.max(100, (int) (reuse * Formulas.calcDexAttackRecoveryFactor(_actor)));
        this.setAttackTask(hits, weapon, recoveryReuse, isSoulshot);
        ...
        if (_actor instanceof Player) {
            ...
            _actor.sendPacket(new SetupGauge(GaugeColor.RED, safeAtkTime + recoveryReuse));
        }
        return hits;
    }
    ...
    private HitHolder[] doAttackHitByDual(Creature target, Weapon weapon, int sAtk, boolean isSoulshot) {
        ...
        int safeAtkTime = Math.max(100, sAtk);
        int dualInterval = Math.max(50, safeAtkTime / 2);
        int recoveryDelay = Math.max(50, (int) (dualInterval * Formulas.calcDexAttackRecoveryFactor(_actor)));
        this.setAttackTask(hits, weapon, recoveryDelay, isSoulshot);
        this._attackTask = ThreadPool.schedule(this::onHitTimer, (long) dualInterval);
        return hits;
    }
    ...
    private HitHolder[] doAttackHitByPole(Creature target, Weapon weapon, int sAtk, boolean isSoulshot) {
        ...
        int safeAtkTime = Math.max(100, sAtk);
        int recoveryDelay = Math.max(100, (int) (safeAtkTime * Formulas.calcDexAttackRecoveryFactor(_actor)));
        this.setAttackTask(hits, weapon, recoveryDelay, isSoulshot);
        this._attackTask = ThreadPool.schedule(this::onHitTimer, (long) safeAtkTime);
        return hits;
    }
