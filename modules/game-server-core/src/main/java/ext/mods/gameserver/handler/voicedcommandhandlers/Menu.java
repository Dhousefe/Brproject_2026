...
public class Menu implements IVoicedCommandHandler
{
	...
	public boolean useVoicedCommand(String command, Player player, String target)
	{
		...
		else if (command.startsWith("offline"))
		{
			...
			if ((player.isInStoreMode() && ConfigOfflineShop.OFFLINE_TRADE_ENABLE) || (player.isCrafting() && ConfigOfflineShop.OFFLINE_CRAFT_ENABLE))
			{
		...
		{
			final String tag = command.substring(5).trim();
			if (!tag.isEmpty())
			{
				final Locale newLocale = Locale.forLanguageTag(tag);
				// forLanguageTag("und") or invalid tags give empty-language Locale; only apply if non-empty.
				if (newLocale != null && !newLocale.getLanguage().isEmpty())
	...
	private void showHtm(Player player)
	{
		...
		TimeZone timeZone = TimeZone.getTimeZone(ConfigProject.TIME_ZONE);
		Calendar currentTime = Calendar.getInstance(timeZone);

		// Defensive: SimpleDateFormat ctor NPEs on a null Locale; fall back to server default.
		final Locale dateLocale = player.getLocale() != null ? player.getLocale() : Locale.getDefault();
		SimpleDateFormat dateFormat = new SimpleDateFormat(ConfigProject.DATE_FORMAT, dateLocale);
		...
		htm.replace("%online%", player.isInStoreMode() ? 0 : World.getInstance().getOnlinePlayerCount() * ConfigProject.FAKE_ONLINE_AMOUNT);
