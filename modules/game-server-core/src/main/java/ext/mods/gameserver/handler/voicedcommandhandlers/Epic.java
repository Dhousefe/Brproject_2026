...
public class Epic implements IVoicedCommandHandler
{
	...
	{
		bosses = IntStream.of(ConfigNpcs.EPIC_BOSS_LIST).boxed().map(id -> NpcData.getInstance().getTemplate(id)).filter(o -> o != null).sorted(Comparator.comparing(NpcTemplate::getLevel)).toList();
	}
	...
	{
		if (!ConfigProject.SHOW_EPIC_HTM)
		{
		...
		for (NpcTemplate npc : list)
		{
			...
			final String teleLink = ConfigProject.EPIC_BOSS_TELEPORT_ENABLED
	...
	{
		if (!ConfigProject.EPIC_BOSS_TELEPORT_ENABLED)
			return;
	...
	private boolean canTeleport(Player player)
	{
		...
		if (TournamentState.isIn(player))
		{
	...
	{
		final int minRange = Math.max(0, ConfigProject.BOSS_TELEPORT_MIN_RANGE);
		final int maxRange = Math.max(minRange, ConfigProject.BOSS_TELEPORT_MAX_RANGE);
	...
	{
		if (ConfigProject.BOSS_TELEPORT_ITEM_ID <= 0 || ConfigProject.BOSS_TELEPORT_ITEM_COUNT <= 0)
			return true;
		
		final long count = ConfigProject.BOSS_TELEPORT_ITEM_COUNT;
		...
		if (!player.destroyItemByItemId(ConfigProject.BOSS_TELEPORT_ITEM_ID, (int) count, true))
			return false;
