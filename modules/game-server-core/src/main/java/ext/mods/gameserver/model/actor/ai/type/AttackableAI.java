	...
	protected void thinkWander()
	{
		...
		else
		{
			_wanderTask = ThreadPool.schedule(() ->
			{
				...
				if (ConfigNpcs.RANDOM_WALK_RATE > 0 && Rnd.get(100) < ConfigNpcs.RANDOM_WALK_RATE)
				{
	...
	protected void onEvtAggression(Creature target, int aggro)
	{
		...
		if (actorClans != null && _actor.getTemplate().getClanRange() > 0)
		{
			...
			_actor.forEachKnownTypeInRadius(Attackable.class, _actor.getTemplate().getClanRange(), called ->
			{
				...
				if (Math.abs(called.getZ() - _actor.getZ()) > 300)
					return;
				
				final List<Quest> clanQuests = called.getTemplate().getEventQuests(EventHandler.CLAN_ATTACKED);
				if (clanQuests.isEmpty())
					return;
				
				if (!GeoEngine.getInstance().canSeeTarget(_actor, called))
					return;
				
				for (Quest quest : clanQuests)
