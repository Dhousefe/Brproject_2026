...
public abstract class ASpawn
{
	...
	protected final long calculateBossRespawnDelayMs(Npc npc)
	{
		if (npc == null || !npc.isRaidBoss())
			return 0L;
		if (!ConfigProject.USE_CONFIG_RAID_GRAND_BOSS_RESPAWN)
			return 0L;
		
		final boolean isGrandBoss = npc instanceof GrandBoss;
		final Map<Integer, long[]> overrides = isGrandBoss ? ConfigProject.GRAND_BOSS_RESPAWN_OVERRIDES : ConfigProject.RAID_BOSS_RESPAWN_OVERRIDES;
		final long[] override = (overrides == null) ? null : overrides.get(npc.getNpcId());
		final long baseMs = (override != null) ? override[0] : (isGrandBoss ? ConfigProject.GRAND_BOSS_RESPAWN_BASE_MS : ConfigProject.RAID_BOSS_RESPAWN_BASE_MS);
		final long randomMs = (override != null) ? override[1] : (isGrandBoss ? ConfigProject.GRAND_BOSS_RESPAWN_RANDOM_MS : ConfigProject.RAID_BOSS_RESPAWN_RANDOM_MS);
		if (baseMs <= 0)
			return 0L;
	...
	public Npc initializeAndSpawn(Npc npc)
	{
		...
		else
		{
			...
			if (ConfigNpcs.CHAMPION_FREQUENCY > 0)
			{
				if (npc instanceof Monster && !getTemplate().cantBeChampion() && npc.getStatus().getLevel() >= ConfigNpcs.CHAMP_MIN_LVL && npc.getStatus().getLevel() <= ConfigNpcs.CHAMP_MAX_LVL && !npc.isRaidRelated() && !npc.hasMaster())
					((Monster) npc).setChampion(Rnd.get(100) < ConfigNpcs.CHAMPION_FREQUENCY);
			}
