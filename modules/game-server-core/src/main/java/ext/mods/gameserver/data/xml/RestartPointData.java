...
public final class RestartPointData implements IXmlReader
{
	...
	public Location getLocationToTeleport(Player player, RestartType teleportType)
	{
		...
		if (random != null)
		{
			try
			{
				// When farm event is running, keep players in the event flow (no local zone respawn).
				if (random.isActive() && FarmEventHooks.get().isEventRunning())
				{
					// fall through — event manages respawn
				}
				else
				{
