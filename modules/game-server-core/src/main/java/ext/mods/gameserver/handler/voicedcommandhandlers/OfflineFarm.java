...
public class OfflineFarm implements IVoicedCommandHandler
{
	...
	public boolean useVoicedCommand(String command, Player player, String args)
	{
		...
		if (!ConfigProject.ENABLE_OFFLINE_FARM_COMMAND)
		{
		...
		if (ConfigProject.DUALBOX_CHECK_MAX_OFFLINEPLAY_PER_IP > 0 || ConfigProject.DUALBOX_CHECK_MAX_OFFLINEPLAY_PREMIUM_PER_IP > 0)
		{
			int offlineCount = 0;
			...
			{
				if (p != null && p.isOfflineFarm() && p.getClient() != null)
				{
					if (p.getClient().getIpAddress().equals(playerIp))
					{
						offlineCount++;
			...
			if (ConfigProject.DUALBOX_CHECK_MAX_OFFLINEPLAY_PER_IP > 0 && offlineCount >= ConfigProject.DUALBOX_CHECK_MAX_OFFLINEPLAY_PER_IP)
			{
			...
			if (ConfigProject.DUALBOX_CHECK_MAX_OFFLINEPLAY_PREMIUM_PER_IP > 0 && player.getPremiumService() > 0 && offlinePremiumCount >= ConfigProject.DUALBOX_CHECK_MAX_OFFLINEPLAY_PREMIUM_PER_IP)
			{
		...
		if (ConfigProject.OFFLINE_FARM_PREMIUM && player.getPremiumService() == 0)
		{
