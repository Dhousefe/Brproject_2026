...
import ext.mods.gameserver.network.serverpackets.ClientSetTime;
...
public class EnterWorld extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (player.isGM())
		{
			...
			if (ConfigProject.SUPER_HASTE)
				SkillTable.getInstance().getInfo(7029, 4).getEffects(player, player);
			
			if (ConfigPlayers.GM_STARTUP_INVULNERABLE && AdminData.getInstance().hasAccess("admin_invul", player.getAccessLevel()))
				player.setInvul(true);
			
			if (ConfigPlayers.GM_STARTUP_INVISIBLE && AdminData.getInstance().hasAccess("admin_hide", player.getAccessLevel()))
				player.getAppearance().setVisible(false);
			
			if (ConfigPlayers.GM_STARTUP_BLOCK_ALL)
				player.setInBlockingAll(true);
			
			if (ConfigPlayers.GM_STARTUP_AUTO_LIST && AdminData.getInstance().hasAccess("admin_gmlist", player.getAccessLevel()))
		...
		if (ConfigServer.ENABLE_COMMUNITY_BOARD && MailBBSManager.getInstance().checkIfUnreadMail(player))
		{
		...
		if (ConfigServer.ENABLE_COMMUNITY_BOARD && clan != null && clan.isNoticeEnabled())
		{
		...
		else if (ConfigServer.SERVER_NEWS)
		{
			final NpcHtmlMessage html = new NpcHtmlMessage(0);
		...
		SafeDisconnectHooks.get().onEnterWorld(player);
		...
		if (ConfigDonation.ENABLE_PIX_MOD && ConfigDonation.DONATION_ENABLED)
			DonationHooks.get().offlinePlayer(player);
		...
		PlayerGodHooks.get().onEnterWorld(player);
		DressMeHooks.get().restore(player);
		if (TournamentHooks.get().isRunning())
		{
			int tourQm = TournamentHooks.get().tutorialQuestionMarkId();
			if (tourQm >= 0)
				player.sendPacket(new TutorialShowQuestionMark(tourQm));
			String lastStart = TournamentHooks.get().lastEvent();
			if (lastStart != null && !lastStart.isEmpty())
			{
				LocalTime startTime = LocalTime.parse(lastStart, DateTimeFormatter.ofPattern("HH:mm"));
				LocalTime now = LocalTime.now();
				int durationMinutes = TournamentHooks.get().getDurationMinutes();
		...
		if (FarmEventHooks.get().isEventRunning())
		{
			int farmQm = FarmEventHooks.get().tutorialQuestionMarkId();
			if (farmQm >= 0)
				player.sendPacket(new TutorialShowQuestionMark(farmQm));
		}
		LevelUpMakerHooks.get().sendQuestionMark(player);
		ext.mods.extensions.hooks.QuestRecommenderHooks.get().sendQuestionMark(player);
		if (hwid.isProtectionOn())
			hwid.enterlog(player, getClient());
		
		// Sincroniza clima Fermata ativo caso haja substituicao do servidor (Zero Regressao para clientes comuns)
		ext.mods.gameserver.data.manager.FermataWeatherManager.getInstance().onPlayerEnterWorld(player);
		
		// Sincroniza relogio e ciclo dia/noite do mundo (ratio 6) com o cliente
