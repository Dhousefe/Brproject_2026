...
public class RaidBossStandard extends MonsterAI
{
	...
	public void onAttacked(Npc npc, Creature attacker, int damage, L2Skill skill)
	{
		...
		if (!ConfigNpcs.RAID_DISABLE_CURSE && attacker.getStatus().getLevel() > (npc.getStatus().getLevel() + 8))
		{
		...
		if (!npc.isInsideZone(ZoneId.PEACE) && !ConfigProject.CATACOMBS_IN_ANY_PERIOD)
		{
	...
	{
		if (!ConfigNpcs.RAID_DISABLE_CURSE && caster.getStatus().getLevel() > (npc.getStatus().getLevel() + 8))
		{
