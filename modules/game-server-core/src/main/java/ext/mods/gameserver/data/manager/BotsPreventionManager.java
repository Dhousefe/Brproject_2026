...
public class BotsPreventionManager
{
	...
	protected int VALIDATION_TIME = ConfigProject.VALIDATION_TIME * 1000;
	...
	public void updateCounter(Creature player, Creature creature)
	{
		if (player instanceof Player killer && creature instanceof Monster)
		{
			...
			int next = Rnd.nextInt(ConfigProject.KILLS_COUNTER_RANDOMIZATION);
			if (ConfigProject.KILLS_COUNTER + next < count)
			{
	...
	private static void validationWindow(Player player)
	{
		...
		StringUtil.append(tb, "<br><br><img src=\"Crest.crest_" + ConfigHexId.SERVER_ID + "_" + (_validation.get(player.getObjectId()).patternId) + "\" width=\"32\" height=\"32\"></td></tr>");
		...
		for (int i = 0; i < container.options.size(); i++)
			StringUtil.append(tb, "<td><button action=\"bypass -h report_" + i + "\" width=32 height=32 back=\"Crest.crest_" + ConfigHexId.SERVER_ID + "_" + (container.options.get(i) + 1500) + "\" fore=\"Crest.crest_" + ConfigHexId.SERVER_ID + "_" + (container.options.get(i) + 1500) + "\"></td>");
	...
	protected void banPunishment(Player player)
	{
		...
		switch (ConfigProject.PUNISHMENT)
		{
			case 0:
			...
			case 1:
				...
				break;
			case 2:
				jailPunishment(player, ConfigProject.PUNISHMENT_TIME * 60);
				break;
			case 3:
	...
	{
		if (player.isOnline())
			player.getPunishment().setType(PunishmentType.JAIL, ConfigProject.PUNISHMENT_TIME);
		else
		{
				...
				PreparedStatement ps = con.prepareStatement(UPDATE_JAIL))
			{
				...
				ps.setLong(2, (delay > 0 ? delay * ConfigProject.PUNISHMENT_TIME * 100 : 0));
