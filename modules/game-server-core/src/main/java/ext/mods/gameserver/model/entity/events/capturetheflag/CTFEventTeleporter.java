...
public class CTFEventTeleporter implements Runnable
{
	...
	public CTFEventTeleporter(Player player, int[] coordinates, boolean fastSchedule, boolean adminRemove)
	{
		...
		ThreadPool.schedule(this, fastSchedule ? 0 : (CTFEvent.getInstance().isStarted() ? ConfigEvents.CTF_EVENT_RESPAWN_TELEPORT_DELAY : ConfigEvents.CTF_EVENT_START_LEAVE_TELEPORT_DELAY) * 1000);
	}
	...
	public void run()
	{
		...
		if ((ConfigEvents.CTF_EVENT_EFFECTS_REMOVAL == 0) || ((ConfigEvents.CTF_EVENT_EFFECTS_REMOVAL == 1) && ((_player.getTeam() == TeamType.NONE) || (_player.isInDuel() && (_player.getDuelState() != DuelState.INTERRUPTED)))))
