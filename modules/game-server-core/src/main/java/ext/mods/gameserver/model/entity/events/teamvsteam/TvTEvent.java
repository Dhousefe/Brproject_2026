...
public class TvTEvent extends Events
{
	...
	public void init()
	{
		...
		_teams[0] = new TvTEventTeam(ConfigEvents.TVT_EVENT_TEAM_1_NAME, ConfigEvents.TVT_EVENT_TEAM_1_COORDINATES);
		_teams[1] = new TvTEventTeam(ConfigEvents.TVT_EVENT_TEAM_2_NAME, ConfigEvents.TVT_EVENT_TEAM_2_COORDINATES);
	}
	
	public boolean startParticipation()
	{
		NpcTemplate tmpl = NpcData.getInstance().getTemplate(ConfigEvents.TVT_EVENT_PARTICIPATION_NPC_ID);
		...
		try
		{
			...
			_npcSpawn.setLoc(ConfigEvents.TVT_EVENT_PARTICIPATION_NPC_COORDINATES[0], ConfigEvents.TVT_EVENT_PARTICIPATION_NPC_COORDINATES[1], ConfigEvents.TVT_EVENT_PARTICIPATION_NPC_COORDINATES[2], ConfigEvents.TVT_EVENT_PARTICIPATION_NPC_COORDINATES[3]);
	...
	public boolean startFight()
	{
		...
		if ((_teams[0].getParticipatedPlayerCount() < ConfigEvents.TVT_EVENT_MIN_PLAYERS_IN_TEAMS) || (_teams[1].getParticipatedPlayerCount() < ConfigEvents.TVT_EVENT_MIN_PLAYERS_IN_TEAMS))
		{
			setState(EventState.INACTIVE);
			
			for (Player players : allParticipants.values())
				players.addItem(ConfigEvents.TVT_EVENT_PARTICIPATION_FEE[0], ConfigEvents.TVT_EVENT_PARTICIPATION_FEE[1], true);
		...
		closeDoors(ConfigEvents.TVT_DOORS_IDS_TO_CLOSE);
		...
		for (TvTEventTeam team : _teams)
		{
			for (Player player : team.getParticipatedPlayers().values())
			{
				if (player != null)
				{
					...
					if (ConfigEvents.TVT_EVENT_ON_KILL.equalsIgnoreCase("title") || ConfigEvents.TVT_EVENT_ON_KILL.equalsIgnoreCase("pmtitle"))
					{
	...
	public String calculateRewards()
	{
		if (_teams[0].getPoints() == _teams[1].getPoints())
		{
			...
			if (ConfigEvents.TVT_REWARD_TEAM_TIE)
			{
				rewardTeam(_teams[0]);
				rewardTeam(_teams[1]);
	...
	private void rewardTeam(TvTEventTeam team)
	{
		for (Player player : team.getParticipatedPlayers().values())
		{
			...
			if (ConfigEvents.TVT_REWARD_PLAYER)
				if (!team.onScoredPlayer(player.getObjectId()))
					continue;
				
			SystemMessage systemMessage = null;
			
			for (IntIntHolder reward : ConfigEvents.TVT_EVENT_REWARDS)
			{
	...
	public void stopFight()
	{
		...
		openDoors(ConfigEvents.TVT_DOORS_IDS_TO_CLOSE);
		...
		for (TvTEventTeam team : _teams)
		{
			for (Player player : team.getParticipatedPlayers().values())
			{
				if (player != null)
				{
					new TvTEventTeleporter(player, ConfigEvents.TVT_EVENT_PARTICIPATION_NPC_COORDINATES, false, false);
					player.clearPoints();
					if (ConfigEvents.TVT_EVENT_ON_KILL.equalsIgnoreCase("title") || ConfigEvents.TVT_EVENT_ON_KILL.equalsIgnoreCase("pmtitle"))
					{
						ThreadPool.schedule(() ->
						{
							...
							player.broadcastTitleInfo();
							player.clearPoints();
						}, ConfigEvents.TVT_EVENT_START_LEAVE_TELEPORT_DELAY * 1000);
					}
				}
			}
	...
	{
		return player.destroyItemByItemId(ConfigEvents.TVT_EVENT_PARTICIPATION_FEE[0], ConfigEvents.TVT_EVENT_PARTICIPATION_FEE[1], true);
	}
	
	public String getParticipationFee()
	{
		int itemId = ConfigEvents.TVT_EVENT_PARTICIPATION_FEE[0];
		int itemNum = ConfigEvents.TVT_EVENT_PARTICIPATION_FEE[1];
	...
	public void onLogout(Player player)
	{
		...
		{
			if (SafeDisconnectHooks.get().isSafeDisconnectActive(player) && ConfigSafeDisconnect.SAFEDISCONNECT_INTEGRATION_TVT)
				return;
			
			if (removeParticipant(player.getObjectId()))
			{
				player.teleportTo((ConfigEvents.TVT_EVENT_PARTICIPATION_NPC_COORDINATES[0] + Rnd.get(101)) - 50, (ConfigEvents.TVT_EVENT_PARTICIPATION_NPC_COORDINATES[1] + Rnd.get(101)) - 50, ConfigEvents.TVT_EVENT_PARTICIPATION_NPC_COORDINATES[2], 0);
	...
	public synchronized void onBypass(String command, Player player)
	{
		...
		if (command.equals("tvt_event_participation"))
		{
			...
			else if (ConfigEvents.DISABLE_ID_CLASSES_TVT.contains(player.getClassId().getId()))
			{
				htmContent = HTMLData.getInstance().getHtm(player, htmlPath + "Class.htm");
				if (htmContent != null)
					npcHtmlMessage.setHtml(htmContent);
			}
			else if ((playerLevel < ConfigEvents.TVT_EVENT_MIN_LVL) || (playerLevel > ConfigEvents.TVT_EVENT_MAX_LVL))
			{
				htmContent = HTMLData.getInstance().getHtm(player, htmlPath + "Level.htm");
				if (htmContent != null)
				{
					npcHtmlMessage.setHtml(htmContent);
					npcHtmlMessage.replace("%min%", String.valueOf(ConfigEvents.TVT_EVENT_MIN_LVL));
					npcHtmlMessage.replace("%max%", String.valueOf(ConfigEvents.TVT_EVENT_MAX_LVL));
				}
			}
			else if ((_teams[0].getParticipatedPlayerCount() == ConfigEvents.TVT_EVENT_MAX_PLAYERS_IN_TEAMS) && (_teams[1].getParticipatedPlayerCount() == ConfigEvents.TVT_EVENT_MAX_PLAYERS_IN_TEAMS))
			{
				htmContent = HTMLData.getInstance().getHtm(player, htmlPath + "TeamsFull.htm");
				if (htmContent != null)
				{
					npcHtmlMessage.setHtml(htmContent);
					npcHtmlMessage.replace("%max%", String.valueOf(ConfigEvents.TVT_EVENT_MAX_PLAYERS_IN_TEAMS));
				}
			}
			else if ((ConfigEvents.TVT_EVENT_MAX_PARTICIPANTS_PER_IP > 0) && !AntiFeedManager.getInstance().tryAddPlayer(AntiFeedManager.TVT_ID, player, ConfigEvents.TVT_EVENT_MAX_PARTICIPANTS_PER_IP))
			{
				htmContent = HTMLData.getInstance().getHtm(player, htmlPath + "IPRestriction.htm");
				if (htmContent != null)
				{
					npcHtmlMessage.setHtml(htmContent);
					npcHtmlMessage.replace("%max%", String.valueOf(AntiFeedManager.getInstance().getLimit(player, ConfigEvents.TVT_EVENT_MAX_PARTICIPANTS_PER_IP)));
				}
			}
		...
		else if (command.equals("tvt_event_remove_participation"))
		{
			...
			if (ConfigEvents.TVT_EVENT_MAX_PARTICIPANTS_PER_IP > 0)
	...
	public boolean onAction(Player player, int objectId)
	{
		...
		if ((playerTeamId != -1) && (targetedPlayerTeamId != -1) && (playerTeamId == targetedPlayerTeamId) && (player.getObjectId() != objectId) && !ConfigEvents.TVT_EVENT_TARGET_TEAM_MEMBERS_ALLOWED)
		{
			player.sendPacket(ActionFailed.STATIC_PACKET);
			return false;
		}
	...
	public boolean onScrollUse(int objectId)
	{
		...
		return isPlayerParticipant(objectId) && !ConfigEvents.TVT_EVENT_SCROLL_ALLOWED;
	}
	...
	public boolean onPotionUse(int objectId)
	{
		...
		return isPlayerParticipant(objectId) && !ConfigEvents.TVT_EVENT_POTIONS_ALLOWED;
	}
	...
	public boolean onItemSummon(int objectId)
	{
		...
		return isPlayerParticipant(objectId) && !ConfigEvents.TVT_EVENT_SUMMON_BY_ITEM_ALLOWED;
	}
	...
	public void onKill(Creature killer, Player player)
	{
		...
		if (attackingPlayer != null)
		{
			...
			if (killerTeamId != -1 && killerTeamId != killedTeamId)
			{
				...
				switch (ConfigEvents.TVT_EVENT_ON_KILL.toLowerCase())
				{
					case "pm":
					...
					case "pmtitle":
						...
						if (ConfigEvents.TVT_EVENT_ON_KILL.equalsIgnoreCase("pmtitle"))
							sysMsgToAllParticipants(killMessage);
						break;
				}
			}
	...
	public void onTeleported(Player player)
	{
		...
		Map<Integer, Integer> buffs = player.isMageClass() ? ConfigEvents.TVT_EVENT_MAGE_BUFFS : ConfigEvents.TVT_EVENT_FIGHTER_BUFFS;
