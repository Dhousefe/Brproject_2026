...
public final class RequestPetition extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (!ConfigPlayers.PETITIONING_ALLOWED)
		{
		...
		if (serverPetitionCount > ConfigPlayers.MAX_PETITIONS_PENDING)
		{
		...
		if (playerPetitionCount > ConfigPlayers.MAX_PETITIONS_PER_PLAYER)
		{
		...
		player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.SUBMITTED_YOUR_S1_TH_PETITION_S2_LEFT).addNumber(playerPetitionCount).addNumber(ConfigPlayers.MAX_PETITIONS_PER_PLAYER - playerPetitionCount));
