...
public class Acp implements IVoicedCommandHandler
{
	...
	{
		if (!ConfigProject.AUTO_POTIONS_ENABLED)
		{
			player.sendMessage(player.getSysString(10_200));
			return false;
		}
		
		if (player.getStatus().getLevel() < ConfigProject.AUTO_POTION_MIN_LEVEL)
		{
			player.sendMessage(player.getSysString(10_188, ConfigProject.AUTO_POTION_MIN_LEVEL));
			return false;
		}
		...
		switch (command)
		{
			...
			default:
				if (command.startsWith("setCp") && ConfigProject.AUTO_CP_ENABLED)
					setAutoPotionValue(command, player, "Cp");
				else if (command.startsWith("setHp") && ConfigProject.AUTO_HP_ENABLED)
					setAutoPotionValue(command, player, "Hp");
				else if (command.startsWith("setMp") && ConfigProject.AUTO_MP_ENABLED)
