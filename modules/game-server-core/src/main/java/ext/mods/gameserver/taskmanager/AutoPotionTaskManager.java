...
public class AutoPotionTaskManager implements Runnable
{
	...
	{
		ThreadPool.scheduleAtFixedRate(this, 0, ConfigProject.ACP_PERIOD);
	}
	...
	public void run()
	{
		...
		if (!PLAYERS.isEmpty())
		{
			...
			{
				if (player == null || player.isAlikeDead() || player.isOnlineInt() != 1 || !ConfigProject.AUTO_POTIONS_IN_OLYMPIAD && player.isInOlympiadMode())
				{
					remove(player);
					continue;
				}
				
				if (ConfigProject.AUTO_CP_ENABLED)
				{
					final boolean restoreCP = ((player.getStatus().getCp() / player.getStatus().getMaxCp()) * 100) <= player.isAcpCp();
					for (int itemId : ConfigProject.AUTO_CP_ITEM_IDS)
					{
