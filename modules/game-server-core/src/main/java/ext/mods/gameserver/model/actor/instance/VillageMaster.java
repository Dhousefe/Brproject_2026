...
public class VillageMaster extends Folk
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		if (command.startsWith("create_clan") && command.length() > 12)
		{
			...
			if (!StringUtil.isValidString(val, ConfigServer.CLAN_ALLY_NAME_TEMPLATE))
			{
		...
		else if (actualCommand.equalsIgnoreCase("create_ally"))
		{
			...
			if (!StringUtil.isValidString(cmdParams, ConfigServer.CLAN_ALLY_NAME_TEMPLATE))
			{
		...
		else if (actualCommand.equalsIgnoreCase("dissolve_clan"))
		{
			...
			if (ConfigClans.CLAN_DISSOLVE_DAYS > 0)
			{
				clan.setDissolvingExpiryTime(System.currentTimeMillis() + ConfigClans.CLAN_DISSOLVE_DAYS * 86400000L);
		...
		else if (command.startsWith("Subclass"))
		{
			...
			switch (cmdChoice)
			{
				...
				case 4:
					...
					if (allowAddition && !ConfigPlayers.SUBCLASS_WITHOUT_QUESTS)
	...
	protected boolean checkQuests(Player player)
	{
		...
		if (ConfigPlayers.SUBCLASS_REQUIRE_MIMIR)
		{
		...
		if (ConfigPlayers.SUBCLASS_REQUIRE_FATE)
		{
	...
	{
		if (ConfigProject.GAME_SUBCLASS_EVERYWHERE)
			return true;
