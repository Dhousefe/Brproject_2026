	...
	public Augmentation generateRandomAugmentation(int lifeStoneLevel, int lifeStoneGrade)
	{
		...
		switch (lifeStoneGrade)
		{
			case AbstractRefinePacket.GRADE_NONE -> {
				if (Rnd.get(1, 100) <= ConfigPlayers.AUGMENTATION_NG_SKILL_CHANCE)
					generateSkill = true;
				if (Rnd.get(1, 100) <= ConfigPlayers.AUGMENTATION_NG_GLOW_CHANCE)
					generateGlow = true;
			}
			
			case AbstractRefinePacket.GRADE_MID -> {
				if (Rnd.get(1, 100) <= ConfigPlayers.AUGMENTATION_MID_SKILL_CHANCE)
					generateSkill = true;
				if (Rnd.get(1, 100) <= ConfigPlayers.AUGMENTATION_MID_GLOW_CHANCE)
					generateGlow = true;
			}
			
			case AbstractRefinePacket.GRADE_HIGH -> {
				if (Rnd.get(1, 100) <= ConfigPlayers.AUGMENTATION_HIGH_SKILL_CHANCE)
					generateSkill = true;
				if (Rnd.get(1, 100) <= ConfigPlayers.AUGMENTATION_HIGH_GLOW_CHANCE)
					generateGlow = true;
			}
			
			case AbstractRefinePacket.GRADE_TOP -> {
				if (Rnd.get(1, 100) <= ConfigPlayers.AUGMENTATION_TOP_SKILL_CHANCE)
					generateSkill = true;
				if (Rnd.get(1, 100) <= ConfigPlayers.AUGMENTATION_TOP_GLOW_CHANCE)
					generateGlow = true;
			}
		}
		
		if (!generateSkill && Rnd.get(1, 100) <= ConfigPlayers.AUGMENTATION_BASESTAT_CHANCE)
