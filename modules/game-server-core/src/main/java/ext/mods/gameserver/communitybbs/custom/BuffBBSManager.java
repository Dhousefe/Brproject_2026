...
public class BuffBBSManager extends BaseBBSManager
{
	...
	private void showGiveBuffsWindow(Player player)
	{
		...
		html = html.replace("%max_schemes%", String.valueOf(ConfigNpcs.BUFFER_MAX_SCHEMES));
	...
	private static String getTypesFrame(Player player, String groupType, String schemeName, boolean singleSelection)
	{
		...
		for (String type : BufferManager.getInstance().getSkillTypes())
		{
			if (player.getPremiumService() == 0)
			{
				if (ConfigProject.PREMIUM_BUFFS_CATEGORY.isEmpty())
					continue;
			}
	...
	{
		if (ConfigNpcs.BUFFER_STATIC_BUFF_COST > 0)
			return list.size() * ConfigNpcs.BUFFER_STATIC_BUFF_COST;
	...
	{
		if (player.getPremiumService() == 0)
			skills.removeIf(skill -> ConfigProject.BUFFS_CATEGORY.contains(BufferManager.getInstance().getAvailableBuff(skill).type()));
	}
