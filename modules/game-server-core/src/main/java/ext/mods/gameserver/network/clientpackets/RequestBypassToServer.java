...
public final class RequestBypassToServer extends L2GameClientPacket
{
	...
	private void runImplInternal()
	{
		...
		if (DonationHooks.get().handleBypass(player, _command))
			return;
		...
		else if (_command.startsWith("questnav"))
		{
			...
			{
				LOGGER.warn("Invalid questnav bypass: [{}]", _command, e);
			}
			return;
		}
		else if (_command.startsWith("questnav2"))
		{
			...
			{
				LOGGER.warn("Invalid questnav2 bypass: [{}]", _command, e);
			}
			return;
		}
		else if (_command.startsWith("questnotify"))
		{
			...
			{
				QuestKillState.setNotifyHtml(player, enable);
				player.sendMessage("Notificação via HTML " + (enable ? "ativada" : "desativada") + ".");
			}
			else if (type.equalsIgnoreCase("chat"))
			{
				QuestKillState.setNotifyChat(player, enable);
		...
		else if (_command.startsWith("setquest"))
		{
			try
			{
				...
				{
					QuestKillState.reset(player, quest.getId(), obj.getNpcId());
				}
				
				QuestKillState.setCompleted(player, quest.getId(), false);
				
				QuestManager.getInstance().showMenuQuest(player, 1);
			}
			catch (Exception e)
			{
				LOGGER.warn("Invalid setquest bypass: [{}]", _command, e);
			}
			return;
		}
		else if (_command.startsWith("dungeon"))
		{
			...
			{
				case "enter":
					DungeonHooks.get().handleEnterDungeonId(player, tokenizer);
					break;
		...
		else if (_command.startsWith("droplist"))
		{
			...
			{
				LOGGER.warn("Invalid droplist bypass: [{}]", _command, e);
			}
			return;
		}
		...
		else if (_command.startsWith("_trans") || _command.startsWith("user_translationmenu"))
		{
			...
			else if (_command.equals("_trans toggle"))
			{
				...
				player.sendMessage("Tradução HTML " + (TranslatorState.isHtmlTranslationEnabled(player) ? "ativada" : "desativada") + ".");
			}
			else if (_command.equals("_trans status"))
			{
				String langName = TranslatorHooks.get().getPlayerLanguage(player);
				player.sendMessage("Idioma atual: " + langName + " | Tradução HTML: " + (TranslatorState.isHtmlTranslationEnabled(player) ? "Ativa" : "Inativa"));
			}
			else
			{
		...
		{
			if (ConfigServer.ENABLE_CUSTOM_BBS)
				CustomCommunityBoard.getInstance().handleCommands(getClient(), _command);
			else if (ConfigServer.ENABLE_COMMUNITY_BOARD)
		...
		else if (_command.startsWith("_match"))
		{
			...
			{
				LOGGER.warn("Invalid _match bypass: [{}]", _command, e);
			}
			return;
		}
		else if (_command.startsWith("_diary"))
		{
			...
			{
				LOGGER.warn("Invalid _diary bypass: [{}]", _command, e);
			}
			return;
		}
		else if (_command.startsWith("arenachange"))
		{
			...
			{
				LOGGER.warn("Invalid arenachange bypass: [{}]", _command, e);
			}
			return;
		}
		...
		else if (_command.startsWith("QuestGatekeeper"))
		{
			...
			{
				LOGGER.warn("Invalid QuestGatekeeper bypass: [{}]", _command, e);
			}
			return;
		}
		else if (_command.startsWith("npcfind_byid"))
		{
			...
			{
				LOGGER.warn("Invalid npcfind_byid bypass: [{}]", _command, e);
			}
			return;
		}
		else if (isBuffShopManagerCommand(_command))
		{
			BuffShopHooks.get().handleBypass(player, _command);
			return;
		}
		...
		else if (_command.startsWith("menu_select"))
		{
			...
			if (target instanceof Npc npc)
			{
				...
				if (!list.isEmpty())
				{
					...
					{
						LOGGER.warn("Invalid menu_select bypass: [{}]", _command, e);
					}
				}
				else
