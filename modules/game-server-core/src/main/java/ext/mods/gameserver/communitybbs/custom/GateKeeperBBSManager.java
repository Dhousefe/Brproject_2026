...
public class GateKeeperBBSManager extends BaseBBSManager
{
	...
	protected void teleport(Player player, int index)
	{
		...
		if (ConfigNpcs.FREE_TELEPORT || player.getStatus().getLevel() <= ConfigNpcs.LVL_FREE_TELEPORT || teleport.getPriceCount() == 0 || player.destroyItemByItemId(teleport.getPriceId(), teleport.getPriceCount(), true))
